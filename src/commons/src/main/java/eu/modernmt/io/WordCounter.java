...
public class WordCounter {
  ...
  private static final Pattern CJK_REGEX =
      Pattern.compile("[\\p{IsHan}\\p{IsKatakana}\\p{IsHiragana}]");
  ...
  /**
   * this private method counts the amount of words in a token delimited by ponctuation symbols,
  ...
  private static int countWordsInToken(String token) {
    ...
    // so increment the word count
    if (prev < token.length() - 1) wordCount++;
