...
public class TokensOutputStream implements Closeable {
  ...
  public static String[] tokens(Sentence sentence, boolean printTags, boolean printPlaceholders) {
    ...
    Iterator<Token> iterator =
        printTags ? sentence.iterator() : new ArrayIterator<>(sentence.getWords());
    ...
    while (iterator.hasNext()) {
      ...
      if (token instanceof Tag) {
        ...
        if (printTags) text = XMLUtils.escapeText(text);
      }
  ...
  public TokensOutputStream(File file, boolean append, boolean printTags, boolean printPlaceholders)
      throws FileNotFoundException {
    this(
        new UnixLineWriter(new FileOutputStream(file, append), UTF8Charset.get()),
        printTags,
        printPlaceholders);
  }
