...
/**
 * XMLFixInputStreamReader wraps an InputStream and filter out invalid XML entities. This is
 * necessary because some tools, for example SDL Trados, creates invalid XML with entities such as
 * &#x1E; &#x1F; etc... This implementation replaces those entities with whitespaces.
 */
    ...
    while (m.find()) {
      ...
      if (!accept(c)) {
        if (matchLength == 5) { // 1 digit
          ...
          for (int i = matchStart + 3; i < matchStart + matchLength - 2; i++) buffer[i] = '0';
