...
public class LanguageIndex {
  ...
  public static class Builder {
    ...
    public Builder addRule(LanguagePattern pattern, Language output)
        throws IllegalArgumentException {
      rules
          .computeIfAbsent(pattern.getLanguage(), k -> new ArrayList<>())
    ...
    public LanguageIndex build(boolean includePivot) {
      Map<LanguageDirection, LanguageBridge> pivots =
          includePivot ? computePivotLanguages() : Collections.emptyMap();
    ...
    private Map<LanguageDirection, LanguageBridge> computePivotLanguages() {
          ...
          if (targets != null) {
            for (LanguageDirection target : targets) {
              if (!source.source.getLanguage().equals(target.target.getLanguage())) {
                ...
                SimpleLanguageDirection pivotedKey =
                    SimpleLanguageDirection.fromLanguageDirection(pivoted);
      ...
      for (LanguageDirection direction : directionToBridge.keySet()) this.add(direction);
  ...
  private LanguageIndex(
      Map<SimpleLanguageDirection, List<LanguageDirection>> index,
      Map<String, List<LanguageRule>> rules,
      Map<LanguageDirection, LanguageBridge> pivots) {
    HashSet<LanguageDirection> languages = new HashSet<>();
    for (List<LanguageDirection> entries : index.values()) languages.addAll(entries);
  ...
  /**
   * Map the input language pair to one that is compatible with the supported ones, trying to adapt
   * language and region if necessary. It does not try to map the reversed language pair, if needed
   * call mapIgnoringDirection()
   *
  ...
  public LanguageDirection mapIgnoringDirection(LanguageDirection pair) {
    ...
    if (cached != null) return cached.reversed();

    LanguageDirection mapped = map(pair);

    if (mapped == null) {
      mapped = map(pair.reversed());
      if (mapped != null) mapped = mapped.reversed();
    }
  ...
  private LanguageDirection search(LanguageDirection language) {
    ...
    for (LanguageDirection entry : entries) {
      if (entry.isEqualOrMoreGenericThan(language)) return entry;
    }
  ...
  private LanguageDirection transform(LanguageDirection language) {
    ...
    if (source == null) source = language.source;
    if (target == null) target = language.target;
  ...
  private Language transform(Language language) {
    ...
    if (rules != null) {
      for (LanguageRule rule : rules) {
        if (rule.match(language)) return rule.getOutputLanguage();
      }
    }
    ...
    return language.getLanguage().equals(language.toLanguageTag())
        ? null
        : new Language(language.getLanguage());
  }
  ...
  private static final class SimpleLanguageDirection {
    ...
    private static SimpleLanguageDirection fromLanguageDirection(LanguageDirection language) {
      return new SimpleLanguageDirection(
          language.source.getLanguage(), language.target.getLanguage());
    }
