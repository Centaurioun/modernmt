...
public final class Language implements Serializable {
  ...
  static String parseScript(String string) {
    if (string.length() == 4 && isAlpha(string)) return toTitleCase(string);
    else return null;
  }

  static String parseRegion(String string) {
    if (string.length() == 2 && isAlpha(string)) return string.toUpperCase();
    else if (string.length() == 3 && isDigit(string)) return string;
    else return null;
  }
  ...
  public static Language fromString(String string) {
    ...
    for (int i = 0; i < strings.length; i++) {
      ...
      if (i == 0) {
        ...
        if (language == null) throw new IllegalArgumentException(string);
        tag.append(language);
      } else {
        if (!skip) {
  ...
  public boolean isEqualOrMoreGenericThan(Language other) {
    ...
    if (region != null && !region.equals(other.region)) return false;
    return true;
  }
