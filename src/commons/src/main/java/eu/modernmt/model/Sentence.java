...
public class Sentence implements Serializable, Iterable<Token> {
  ...
  public void addAnnotations(Set<Annotation> annotations) {
    if (this.annotations == null) this.annotations = new HashMap<>(annotations.size());

    for (Annotation annotation : annotations) this.annotations.put(annotation.getId(), annotation);
  }
  ...
  private static String combineSpace(String leftSpace, String rightSpace) {
    if (leftSpace == null) return rightSpace;

    if (rightSpace == null) return leftSpace;

    // both leftSpace and rightSpace are not null

    // leftSpace is equal to rightSpace; hence, return only one of them
    if (leftSpace.equals(rightSpace)) return leftSpace;
  ...
  public static String getSpaceBetweenTokens(
      String previousSpace, Token leftToken, Token rightToken) {
    ...
    if (leftToken instanceof Tag) {
      if (rightToken instanceof Tag) {
        ...
        // Tag-Word
        if (!((Word) rightToken).isLeftSpaceRequired()) space = null;
      }
    } else {
      if (rightToken instanceof Tag) {
        // Word-Tag
        if (!((Word) leftToken).isRightSpaceRequired()) space = null;
        else space = rightToken.getLeftSpace();
      } else {
        // Word-Word
        space = combineSpace(space, rightToken.getLeftSpace());
        if (space == null
            && (((Word) leftToken).hasHiddenRightSpace()
                || ((Word) rightToken).hasHiddenLeftSpace())) {
          space = " ";
        }
      }
    }
  ...
  private String toXMLStrippedString(boolean printPlaceholders) {
    ...
    for (Token token : this) {
      ...
      if (token instanceof Word) {
        ...
        String text =
            printPlaceholders || !token.hasText() ? token.getPlaceholder() : token.getText();
        builder.append(text);
  ...
  private String toXMLString(boolean printPlaceholders) {
    ...
    for (Token token : this) {
      ...
      if (token.hasRightSpace()) builder.append(token.getRightSpace());
    }
