...
public class CompactFileCorpus extends BaseMultilingualCorpus {
  ...
  private static class CompactReader implements TUReader {
    ...
    private TranslationUnit parse(String metadata, String source, String target)
        throws IOException {
      String[] parts = metadata.split(",", 3);
      if (parts.length < 2) throw new IOException("Invalid metadata found: " + metadata);

      Date timestamp = null;
      try {
        long date = Long.parseLong(parts[0]);
        if (date > 0) timestamp = new Date(date);
      } catch (NumberFormatException e) {
        throw new IOException("Invalid metadata found: " + metadata, e);
      }

      LanguageDirection language =
          cachedLanguagePairs.computeIfAbsent(
              parts[1],
              key -> {
                String[] langs = key.split(" ");
                return new LanguageDirection(
                    Language.fromString(langs[0]), Language.fromString(langs[1]));
              });
  ...
  private static class CompactWriter implements TUWriter {
    ...
    private static String encodeMetadata(TranslationUnit tu) {
      ...
      metadata.append(',');
      metadata
          .append(tu.language.source.toLanguageTag())
          .append(' ')
