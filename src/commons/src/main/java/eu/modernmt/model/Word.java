...
public class Word extends Token {
  ...
  // true if a space is required between the previous Word and this Word, according to the
  // (de)tokenization rules
  private boolean leftSpaceRequired;
  // true if a space is required between this Word and the next Word, according to the
  // (de)tokenization rules
  private boolean rightSpaceRequired;
  private String xmlEscapedString = null;

  // the attribute hasHiddenLeftSpace is:
  // - true, if between this token and the closest left Word should be present a space (according to
  // the (de)tokenization modules)),
  //        but the space is not actually present because there is a Token in-between
  // - false, otherwise
  // The attribute is set during the creation of the source sentence, and it is projected from the
  // source to the target by the WhiteSpaceProjector
  ...
  public Word(
      ...
      String leftSpace,
      String rightSpace,
      boolean leftSpaceRequired,
      boolean rightSpaceRequired) {
  ...
  public String toString(boolean escape) {
    if (escape) {
      if (xmlEscapedString == null) xmlEscapedString = XMLUtils.escapeText(super.toString());
