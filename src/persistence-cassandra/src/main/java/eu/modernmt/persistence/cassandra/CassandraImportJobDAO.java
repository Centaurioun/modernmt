 ...
 *
 * <p>A CassandraImportJobDAO object offers methods for performing CRUD operations on ImportJob
 * objects when connected to a Cassandra Database.
 */
public class CassandraImportJobDAO implements ImportJobDAO {
  ...
  /**
   * This method creates a CassandraImportJobDao that will communicate with the Cassandra DB under
   * analysis using a specific connection
   *
   * @param connection the Cassandra Connection that the DAO will employ to deal with the ImportJob
   *     CRUD operations.
   */
  ...
  /**
   * This method stores a ImportJob object in the DB with a new, sequentially generated ID
   *
  ...
  public ImportJob store(ImportJob job) throws PersistenceException {
    ...
    Object[] values = {
      id, job.getMemory(), job.getBegin(), job.getEnd(), job.getDataChannel(), job.getSize()
    };
    BuiltStatement statement =
        QueryBuilder.insertInto("import_jobs").values(columns, values).ifNotExists();
  ...
  /**
   * This method receives a unique UUID for an ImportJob, uses it to extract the corresponding ID
   * employed in the DB and retrieves the relative ImportJob from the importjobs table
   *
   * @param uuid the ID of the ImportJob object to retrieve, in the UUID format
   * @return the ImportJob object with the given ID, if there is one in the DB; else, this method
   *     returns null
  ...
  public ImportJob retrieve(UUID uuid) throws PersistenceException {
    ...
    BuiltStatement statement =
        QueryBuilder.select().from("import_jobs").where(QueryBuilder.eq("id", id));
  ...
  /**
   * This method reads a row returned by a query to the importjobs table in the Cassandra DB and
   * creates a new ImportJob object from its fields
   *
