...
/**
 * Created by andrearossi on 08/03/17. A CassandraDatabase object represents the access point to a
 * Cassandra DB instance. It offers methods for establishing connections with the DB and to get DAO
 * objects for the entities it stores.
 */
public class CassandraDatabase extends Database {
  ...
  /**
   * This constructor builds an access point to a Cassandra DB and, in particular, to one of its
   * keyspaces
   *
  ...
  /**
   * This method creates an access point to the database to work with. If the current cluster objec
   * is already initialized, the method closes it and rebuilds it from scratch.
   */
   ...
   *
   * @param cached
   * @return A CassandraConnection object, that can be used to establish a communication Session
   *     with the DB
  ...
  /**
   * This method establishes a new connection to the DB and drops the current keyspace (with all its
   * tables). If the keyspace has already been dropped, the method does nothing.
   *
   * @throws PersistenceException
   */
  ...
  /**
   * This method establishes a new connection to the DB and uses it to create - a new keyspace - a
   * new memories table - a new importjobs table - a new table_counters table, - a new
   * initialization metadata table, with an entry for memory and another one for importjobs and with
   * an entry in the initialization metadata table
   *
   * @throws PersistenceException
   */
  @Override
  public void create() throws PersistenceException {
    ...
    try {
      ...
      SimpleStatement createCountersTable =
          new SimpleStatement(
              "CREATE TABLE IF NOT EXISTS "
                  + COUNTERS_TABLE
                  + " (table_id int PRIMARY KEY, table_counter bigint);");

      SimpleStatement createMemoriesTable =
          new SimpleStatement(
              "CREATE TABLE IF NOT EXISTS "
                  + MEMORIES_TABLE
                  + " (id bigint PRIMARY KEY, owner_msb bigint, owner_lsb bigint, name varchar);");

      SimpleStatement createImportJobsTable =
          new SimpleStatement(
              "CREATE TABLE IF NOT EXISTS "
                  + IMPORT_JOBS_TABLE
                  + " (id bigint PRIMARY KEY, memory bigint, size int, \"begin\" bigint, end"
                  + " bigint, data_channel smallint);");
  ...
  public void testConnection() throws PersistenceException {
    ...
    try {
      ...
      if (connection.session.isClosed()) throw new PersistenceException("connection closed");
    } finally {
      IOUtils.closeQuietly(connection);
    }
  }
