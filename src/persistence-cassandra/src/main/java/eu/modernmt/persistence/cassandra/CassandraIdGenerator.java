...
/**
 * This class provides static methods that generate sequential integer IDs for objects to store in
 * our Cassandra DB.
 */
public class CassandraIdGenerator {

  /**
   * This method generates a new ID for a new object that must be stored in a certain table. The new
   * IDs are long and are generated in a sequential way.
   *
   ...
   */
  public static long generate(CassandraConnection connection, int tableId)
      throws PersistenceException {
    ...
    BuiltStatement get =
        QueryBuilder.select("table_counter")
    ...
    while (true) {
      ...
      BuiltStatement set =
          QueryBuilder.update(CassandraDatabase.COUNTERS_TABLE)
      ...
      if (CassandraUtils.checkedExecute(connection, set).wasApplied()) return oldCount + 1L;
    }
  }

  /**
   * This method updates the current counter for a table to a given value if it is greater than the
   * current counter for that table
   *
   * @param connection the current connection with the database
   * @param newCounter the new memories counter (if it is greater than the current one)
   * @throws PersistenceException
   */
  public static boolean advanceCounter(CassandraConnection connection, int tableID, long newCounter)
      throws PersistenceException {
    ...
    while (true) {
      ...
      if (counter >= newCounter) return wasApplied;
    }
  }

  /**
   * This method creates the necessary statements to store in the counters_table a new entry for
   * each table created during the database initialization
   *
   * @param connection the current connection with the database
   * @param tableIds the IDs of the tables in the DB
   * @throws PersistenceException
   */
  public static void initializeTableCounter(CassandraConnection connection, int[] tableIds)
      throws PersistenceException {
    String[] columns = {"table_id", "table_counter"};
    for (int table_id : tableIds) {
      Object[] values = {table_id, 0};
      BuiltStatement built =
          QueryBuilder.insertInto(CassandraDatabase.COUNTERS_TABLE)
