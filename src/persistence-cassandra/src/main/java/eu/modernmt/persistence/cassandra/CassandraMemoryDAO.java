 ...
 *
 * <p>A CassandraMemoryDAO object offers methods for performing CRUD operations on Memory objects
...
public class CassandraMemoryDAO implements MemoryDAO {
  ...
  /**
   * This method creates a CassandraMemoryDAO that will communicate with the Cassandra DB under
   * analysis using a specific connection
   *
   * @param connection the Cassandra Connection that the DAO will employ to deal with the Memory
   *     CRUD operations.
   */
  ...
  /**
   * This method retrieves a Memory object in a Cassandra DB by the ID it was stored with.
   *
   * @param id the ID of the Memory object to retrieve
   * @return the Memory object stored with the passed id, if there is one in the DB; else, this
   *     method returns null
  ...
  /**
   * This method reads a row returned by a query to the memories table in the Cassandra DB and
   * creates a new Memory object from its fields
   *
  ...
  private static Memory read(Row row) throws PersistenceException {
    ...
    try {
      ...
      throw new PersistenceException(
          "there is no registered codec to convert the underlying CQL type to a long", e);
    }
  }

  /**
   * This method retrieves from the Cassandra DB all the Memory objects the ids of which are
  ...
  /**
   * This method retrieves from the Cassandra DB all the Memory objects stored in the corresponding
   * table
   *
  ...
  public Collection<Memory> retrieveAll() throws PersistenceException {
    ...
    BuiltStatement statement = QueryBuilder.select().from(CassandraDatabase.MEMORIES_TABLE).where();
    ResultSet result = CassandraUtils.checkedExecute(connection, statement);

    for (Row row : result.all()) list.add(read(row));

    return list;
  }

  /**
   * This method stores a Memory object in the DB with a new, sequentially generated ID
   *
   ...
   *
   * @param memory the Memory object to store in the DB
   * @param forceId if it is true, then this method tries to store memory with its ID. Else it uses
   *     a new, sequentially generated ID.
   * @return if the memory was successfully stored, the method returns memory itself (with its ID
   *     update to the new one if forceId was false). Else, throws an exception.
   * @throws PersistenceException if couldn't insert the importjob in the DB
   */
  @Override
  public Memory store(Memory memory, boolean forceId) throws PersistenceException {
    ...
    BuiltStatement statement =
        QueryBuilder.insertInto(CassandraDatabase.MEMORIES_TABLE)
  ...
  /**
   * This method receives a Memory object and stores it in the DB overwriting an existing row with
   * same ID. If in the DB there is no row with that ID nothing happens.
   *
   * @param memory the Memory object to store in the DB in place of an already existing one
   * @return the same memory object passed as a parameter, if the overwrite is successful (if an
   *     object with that ID was already in the DB) or null if the overwrite was not successful.
   * @throws PersistenceException
   */
  @Override
  public Memory update(Memory memory) throws PersistenceException {
    BuiltStatement built =
        QueryBuilder.update(CassandraDatabase.MEMORIES_TABLE)
    ...
    if (result.wasApplied()) return memory;
    else return null;
  }
   ...
   * @return True if the object was successfully deleted; False if no object with the passed ID
   *     could be found.
   * @throws PersistenceException
   */
  @Override
