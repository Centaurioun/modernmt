...
public class SentenceBatchScheduler extends AbstractScheduler<SentenceBatchScheduler.JobImpl> {
  ...
  @Override
  public TranslationLock schedule(
      LanguageDirection direction, TranslationSplit[] splits, ScoreEntry[] suggestions)
      throws DecoderUnavailableException {
    CountDownTranslationLock lock = new CountDownTranslationLock(splits.length);
    for (TranslationSplit split : splits) split.setLock(lock);
  ...
  @Override
  public TranslationLock schedule(LanguageDirection direction, TranslationSplit split)
      throws DecoderUnavailableException {
  ...
  public static class JobImpl implements Scheduler.Job, Comparable<JobImpl> {
    ...
    JobImpl(LanguageDirection direction, TranslationSplit[] splits, ScoreEntry[] suggestions) {
      this(
          ...
          suggestions != null && suggestions.length > 0 ? Arrays.asList(suggestions) : null);
    }

    private JobImpl(
        LanguageDirection direction, List<TranslationSplit> splits, List<ScoreEntry> suggestions) {
    ...
    public void onStartWaitingInQueue(int queueSize) {
      ...
      for (TranslationSplit split : splits) split.onStartWaitingInQueue(queueSize, this.timestamp);
    }
    ...
    public boolean isAlignmentJob() {
      for (TranslationSplit split : splits) {
        if (split.reference == null) return false;
      }
    ...
    public int compareTo(JobImpl o) {
      if (priority == o.priority) return Long.compare(timestamp, o.timestamp);
      else return Integer.compare(priority.intValue, o.priority.intValue);
    }
  }
}
