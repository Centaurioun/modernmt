...
public class DefaultQueryBuilder implements QueryBuilder {
  ...
  @Override
  public Query getByMatch(
      DocumentBuilder builder,
      long memory,
      LanguageDirection language,
      String previousSentence,
      String previousTranslation) {
    return getByMatchHash(
        builder, memory, HashGenerator.hash(language, previousSentence, previousTranslation));
  }

  public Query getByMatchHash(DocumentBuilder builder, long memory, String hash) {
    PhraseQuery hashQuery = new PhraseQuery();
    for (String h : hash.split(" ")) hashQuery.add(builder.makeHashTerm(h));
  ...
  @Override
  public Query getByTuid(
      DocumentBuilder builder, long memory, LanguageDirection language, String tuid) {
  ...
  public Query getByTuidHash(DocumentBuilder builder, long memory, String hash) {
    PhraseQuery hashQuery = new PhraseQuery();
    for (String h : hash.split(" ")) hashQuery.add(builder.makeTuidHashTerm(h));
  ...
  @Override
  public Query bestMatchingSuggestion(
      DocumentBuilder builder,
      Analyzer analyzer,
      UUID user,
  ...
  protected static BooleanQuery makeTermsQuery(
      DocumentBuilder builder, LanguageDirection direction, Sentence sentence, Analyzer analyzer) {
  ...
  private static void loadTerms(
      String fieldName, Sentence sentence, Analyzer analyzer, BooleanQuery output) {
