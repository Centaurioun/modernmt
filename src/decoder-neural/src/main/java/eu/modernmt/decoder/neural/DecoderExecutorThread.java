    ...
    while ((job = take()) != null) {
      ...
      if (!splits.isEmpty()) {
        ...
        try {
          ...
          for (TranslationSplit split : splits) split.onTranslationBegin(timestamp);

          LanguageDirection language = job.getLanguageDirection();
          if (job.isAlignmentJob()) executor.align(decoder, language, splits);
          else executor.translate(decoder, language, splits, job.getSuggestions());
        } catch (Throwable e) {
          for (TranslationSplit split : job.getTranslationSplits()) split.setException(e);
        } finally {
          if (decoder != null) queue.release(decoder);
        }
      }
    }
  }
