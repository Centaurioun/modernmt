...
public class LuceneTranslationMemory implements TranslationMemory {
  ...
  public LuceneTranslationMemory(File indexPath, Rescorer rescorer, int minQuerySize)
      throws IOException {
    this(FSDirectory.open(forceMkdir(indexPath)), rescorer, minQuerySize);
  }

  public LuceneTranslationMemory(Directory directory, Rescorer rescorer, int minQuerySize)
      throws IOException {
    this(
        directory,
  ...
  public LuceneTranslationMemory(
      File indexPath,
      ...
      throws IOException {
    this(
        FSDirectory.open(forceMkdir(indexPath)),
        documentBuilder,
        queryBuilder,
        rescorer,
        analyzerFactory,
  ...
  public LuceneTranslationMemory(
    ...
    IndexWriterConfig indexConfig =
        new IndexWriterConfig(
            ...
            new DelegatingAnalyzerWrapper(PER_FIELD_REUSE_STRATEGY) {
              @Override
              protected Analyzer getWrappedAnalyzer(String fieldName) {
                if (documentBuilder.isHashField(fieldName))
                  return analyzerFactory.createHashAnalyzer();
                else return analyzerFactory.createContentAnalyzer();
              }
            });
  ...
  public ScoreEntry[] search(
      ...
      throws IOException {
    Analyzer analyzer =
        this.queryBuilder.isLongQuery(source.getWords().length)
            ? longQueryAnalyzer
            : shortQueryAnalyzer;
    Query query =
        this.queryBuilder.bestMatchingSuggestion(
            documentBuilder, analyzer, user, direction, source, contextVector);
    ...
    if (rescorer != null) entries = rescorer.rescore(direction, source, entries, contextVector);
  ...
  public synchronized void optimize() throws IOException {
    ...
    logger.info(
        "Memory forced merge completed in "
            + (elapsed / 1000.)
            + "s "
            + "(deleted-docs = "
            + reader.numDeletedDocs()
  ...
  public synchronized void onDataReceived(DataBatch batch) throws IOException {
    ...
    try {
      ...
      if (!success) this.indexWriter.rollback();
    }
  }
  ...
  private void onTranslationUnitsReceived(Collection<TranslationUnitMessage> units)
      throws IOException {
      ...
      if (currentPosition == null || currentPosition < unit.channelPosition) {
        if (unit.update) {
          ...
          if (unit.previousSentence != null && unit.previousTranslation != null)
            deleteQuery =
                this.queryBuilder.getByMatch(
                    documentBuilder,
                    unit.memory,
                    unit.value.language,
                    unit.previousSentence,
                    unit.previousTranslation);
          else if (unit.value.tuid != null)
            deleteQuery =
                this.queryBuilder.getByTuid(
                    documentBuilder, unit.memory, unit.value.language, unit.value.tuid);

          if (deleteQuery != null) this.indexWriter.deleteDocuments(deleteQuery);
        }
  ...
  public synchronized void close() throws IOException {
    ...
    try {
      if (this._indexReader != null) this._indexReader.close();
    } catch (IOException e) {
      error = e;
    }
    ...
    try {
      if (this.indexDirectory != null) this.indexDirectory.close();
    } catch (IOException e) {
      if (error == null) error = e;
    }
