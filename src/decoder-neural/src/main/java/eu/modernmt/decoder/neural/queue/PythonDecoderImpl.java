...
public class PythonDecoderImpl extends PythonProcess implements PythonDecoder {
  ...
  public static class Builder implements PythonDecoder.Builder {
    ...
    public Builder(
        String pythonExec, File pythonModule, String main, String[] extraArgs, File model) {
    ...
    private PythonDecoderImpl start(int gpu) throws IOException {
      ...
      if (main != null) command.add(main);

      command.add(model.getAbsolutePath());

      if (extraArgs != null && extraArgs.length > 0) command.addAll(Arrays.asList(extraArgs));
      ...
      try {
        ...
        if (!success) IOUtils.closeQuietly(process);
      }
    }
  }
  ...
  public Translation align(LanguageDirection direction, Sentence sentence, String[] translation)
    ...
    return this.translate(
        sentences, serialize(direction, new Sentence[] {sentence}, null, translations))[0];
  }

  @Override
  public Translation[] align(
      LanguageDirection direction, Sentence[] sentences, String[][] translations)
      throws DecoderException {
    return this.translate(sentences, serialize(direction, sentences, null, translations));
  }

  private synchronized Translation[] translate(Sentence[] sentences, String payload)
      throws DecoderException {
    if (!isAlive()) throw new DecoderUnavailableException("Neural decoder process not available");
  ...
  private Translation[] deserialize(String response, Sentence[] sentences)
    ...
    if (success) {
      ...
      for (int i = 0; i < translations.length; i++) {
        ...
        Alignment alignment =
            jsonAlignment == null ? null : parseAlignment(jsonAlignment.getAsJsonArray());
      ...
      if (json.has("msg")) message = json.get("msg").getAsString();

      throw (message == null)
          ? new DecoderException(type)
          : new DecoderException(type + " - " + message);
    }
  }

  private static Alignment parseAlignment(JsonArray array) {
    if (array.size() == 0) return new Alignment(new int[0], new int[0]);
