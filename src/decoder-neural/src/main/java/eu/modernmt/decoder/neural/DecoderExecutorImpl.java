...
public class DecoderExecutorImpl implements DecoderExecutor {
  ...
  @Override
  public void align(
      PythonDecoder decoder, LanguageDirection language, List<TranslationSplit> splits)
    ...
    int i = 0;
    for (TranslationSplit split : splits) split.setTranslation(result[i++]);
  }

  @Override
  public void translate(
      PythonDecoder decoder,
      LanguageDirection language,
      List<TranslationSplit> splits,
      Collection<ScoreEntry> suggestions)
      throws DecoderException {
    Sentence[] sentences = mergeSentences(splits);
    ...
    int i = 0;
    for (TranslationSplit split : splits) split.setTranslation(translations[i++]);
  }

  private static Sentence[] mergeSentences(List<TranslationSplit> splits) {
    Sentence[] sentences = new Sentence[splits.size()];
    for (int i = 0; i < sentences.length; i++) sentences[i] = splits.get(i).sentence;
    return sentences;
  }
  ...
  private static String[][] mergeReferences(List<TranslationSplit> splits) {
    ...
    int i = 0;
    for (TranslationSplit split : splits) result[i++] = split.reference;
