...
public class F1BleuRescorer implements Rescorer {
  ...
  @Override
  public ScoreEntry[] rescore(
      LanguageDirection direction, Sentence input, ScoreEntry[] entries, ContextVector context) {
    ...
    for (ScoreEntry entry : entries) {
      ...
      if (expansion > MAX_SUGGESTION_EXPANSION) entry.score = -1;
    }

    // Compute F1-BLEU score
    for (ScoreEntry entry : entries) {
      if (entry.score >= 0) entry.score = calculator.calc(entry.sentenceTokens);
    }
    ...
    if (context != null && context.size() > 0) {
      for (ContextVector.Entry ce : context) contextScores.put(ce.memory.getId(), ce.score);
    }
