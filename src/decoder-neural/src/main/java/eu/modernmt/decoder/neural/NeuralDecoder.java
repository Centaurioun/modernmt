...
public class NeuralDecoder extends Decoder implements DataListenerProvider {
  ...
  protected NeuralDecoder(File model, DecoderConfig config, DecoderInitializer init)
    ...
    this.decoderQueue =
        this.echoServer
            ? new EchoServerDecoderQueue()
            : init.createDecoderQueue(config, modelConfig, model);
  ...
  public void setListener(DecoderListener listener) {
    if (decoderQueue != null) decoderQueue.setListener(listener);
  ...
  public Translation translate(
      Priority priority, UUID user, LanguageDirection direction, Sentence text, long timeout)
      throws DecoderException {
  ...
  public Translation translate(
      ...
      long timeout)
      throws DecoderException {
    if (!isLanguageSupported(direction)) throw new UnsupportedLanguageException(direction);

    if (!text.hasWords()) return Translation.emptyTranslation(text);
    ...
    if (suggestions != null && suggestions[0].score == 1.f) { // align
      TranslationSplit split =
          new TranslationSplit(priority, text, suggestions[0].translationTokens, timeout);
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        ...
        StringBuilder log =
            new StringBuilder(
                "Translation received from neural decoder:\n"
                    + "   sentence = "
                    + sourceText
                    + "\n"
                    + "   translation = "
        ...
        if (suggestions != null) {
          for (ScoreEntry entry : suggestions) log.append("      ").append(entry).append('\n');
        }
  ...
  protected ScoreEntry[] lookup(
      UUID user, LanguageDirection direction, Sentence text, ContextVector contextVector)
      throws DecoderException {
  ...
  public void test() throws DecoderException {
    ...
    if (decoderQueue.availability() < 1) throw new DecoderException("No decoder process available");

    long now = System.currentTimeMillis();
    if (now - lastSuccessfulTranslation < 5000L) return;
    ...
    synchronized (this) {
      ...
      try {
        ...
        if (decoder != null) decoderQueue.release(decoder);
      }
    }
  }
