...
public class TranslationJoiner {
  ...
  public static Translation join(Sentence originalSentence, TranslationSplit[] splits)
    ...
    if (splits.length == 0) {
      ...
      if (originalSentence != sentence)
        translation =
            new Translation(
                translation.getWords(), originalSentence, translation.getWordAlignment());
  ...
  private static Translation doJoin(Sentence originalSentence, TranslationSplit[] splits)
      throws DecoderException {
    ...
    for (TranslationSplit split : splits) {
      ...
      // Alignment
      if (alignment != null) alignment.append(sentence, translation);
    }

    Translation translation;
    if (alignment == null) translation = new Translation(words.build(), originalSentence, null);
    else translation = new Translation(words.build(), originalSentence, alignment.build());
  ...
  private static int wordCount(TranslationSplit[] splits) throws DecoderException {
    int count = 0;
    for (TranslationSplit split : splits) count += split.getTranslation().getWords().length;
    return count;
  }
  ...
  private static int alignmentCount(TranslationSplit[] splits) throws DecoderException {
    ...
    for (TranslationSplit split : splits) {
      Translation translation = split.getTranslation();
      if (translation.hasAlignment()) count += translation.getWordAlignment().size();
    }
  ...
  private static class WordsJoiner {
    ...
    public void append(Word[] piece) {
      ...
      // force the right space between the existing words and the appended words; no space if there
      // are not existing words
      if (index != 0) {
