...
public class SentenceSplitter {
  ...
  private static int count(Sentence sentence) {
    ...
    for (int i = 0; i < words.length; i++) {
      if (words[i].isSentenceBreak() || i == words.length - 1) result++;
    }
  ...
  public static List<Sentence> split(
      Sentence sentence, boolean includeTags, int minLength, int maxLength) {
    ...
    if (split.size() > 0) output.add(split.toSentence());

    return output;
  }

  private static Pattern PUNCTUATION_WORD =
      Pattern.compile("[\\s\\p{IsPunctuation}]+", Pattern.UNICODE_CHARACTER_CLASS);
  ...
  private static int getBestSplitIndex(Sentence sentence) {
    ...
    for (int i = pivot; i >= 0; i--) {
      ...
      if (PUNCTUATION_WORD.matcher(word).matches()) return i;
  ...
  private static void split(
      Sentence sentence, int index, SentenceSplit split, List<Sentence> output) {
    int i = 0;
    for (Token token : sentence) {
      split.append(token);
