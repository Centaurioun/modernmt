...
public class Preprocessor implements Closeable {
  ...
  public Sentence[] process(LanguageDirection language, String[] batch)
      throws ProcessingException, InterruptedException {
    return process(language, batch, null);
  }

  public List<Sentence> process(LanguageDirection language, List<String> batch)
      throws ProcessingException, InterruptedException {
    return process(language, batch, null);
  }
  ...
  public Sentence[] process(LanguageDirection language, String[] batch, Options options)
      throws ProcessingException, InterruptedException {
  ...
  public List<Sentence> process(LanguageDirection language, List<String> batch, Options options)
      throws ProcessingException, InterruptedException {
    Map<String, Object> metadata = getMetadata(options);
    Sentence[] result =
        this.executor.processBatch(
            metadata, language, batch.toArray(new String[0]), new Sentence[batch.size()]);
    return Arrays.asList(result);
  }

  public Sentence process(LanguageDirection language, String text, Options options)
  ...
  public void close() {
    ...
    try {
      if (!this.executor.awaitTermination(1, TimeUnit.SECONDS)) this.executor.shutdownNow();
  ...
  private static XMLPipelineBuilder<String, Sentence> getDefaultBuilder() throws IOException {
    ...
    try {
      ...
      if (stream == null) throw new Error("Default preprocessor definition not found: " + xmlPath);
