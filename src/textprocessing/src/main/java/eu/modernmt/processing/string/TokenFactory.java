 ...
 *
 * <p>A TokenFactory is an object that is able to build Tokens. Each type of Token requires a
 * specific implementation of TokenFactory.
 */
public interface TokenFactory {

  /** A WORD_FACTORY is an implementation of Token Factory that creates Words */
  TokenFactory WORD_FACTORY =
      new TokenFactory() {
        @Override
        public Token build(
   ...
   * @param position int conveying the amount of WORD tokens already created (it is meaningful only
   *     if the token to create is a TAG
