...
public class LanguageToolTokenAnnotator implements BaseTokenizer.Annotator {
  ...
  private static final Map<Language, Class<? extends LanguageToolTokenizer>> TOKENIZERS =
      new HashMap<>();
  ...
  public static LanguageToolTokenAnnotator forLanguage(Language language)
      throws UnsupportedLanguageException {
    Class<? extends LanguageToolTokenizer> tokenizerClass = TOKENIZERS.get(language);
    if (tokenizerClass == null) throw new UnsupportedLanguageException(language);
  ...
  public static void annotate(TokenizedString tokenizedString, List<String> tokens) {
    ...
    for (String token : tokens) {
      ...
      if (stringIndex <= length) tokenizedString.setWord(tokenPos, tokenPos + tokenLength);
    }
  }
}
