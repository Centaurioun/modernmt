...
public class BretonWordTokenizer extends WordTokenizer {
  ...
  /**
   * Tokenizes just like WordTokenizer with the exception that "c’h" is not split. "C’h" is
   * considered as a letter in breton (trigraph) and it occurs in many words. So tokenizer should
   * not split it. Also split things like "n’eo" into 2 tokens only "n’" + "eo".
   *
   * @param text - Text to tokenize
   * @return List of tokens.
   *     <p>Note: a special string ##BR_APOS## is used to replace apostrophes during tokenizing.
   */
  @Override
  public List<String> tokenize(final String text) {
    ...
    String replaced =
        text.replaceAll("([Cc])['’‘ʼ]([Hh])", "$1\u0001\u0001BR_APOS\u0001\u0001$2")
