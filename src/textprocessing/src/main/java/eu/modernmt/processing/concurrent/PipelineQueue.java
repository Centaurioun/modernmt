...
class PipelineQueue<P, R> {
  ...
  private final ConcurrentHashMap<
          LanguageDirection, ConcurrentLinkedQueue<ProcessingPipeline<P, R>>>
  ...
  public ProcessingPipeline<P, R> get(LanguageDirection language) throws ProcessingException {
    ProcessingPipeline<P, R> pipeline =
        pipelines.computeIfAbsent(language, k -> new ConcurrentLinkedQueue<>()).poll();

    if (pipeline == null) pipeline = builder.newPipeline(language.source, language.target);

    return pipeline;
  }

  public void release(LanguageDirection language, ProcessingPipeline<P, R> pipeline) {
    pipelines.computeIfAbsent(language, k -> new ConcurrentLinkedQueue<>()).offer(pipeline);
  }
}
