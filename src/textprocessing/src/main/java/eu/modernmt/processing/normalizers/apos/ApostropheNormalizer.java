    ...
    return cache.computeIfAbsent(
        ...
        (key) -> {
          ...
          try {
            ...
            if (stream == null) throw new UnsupportedLanguageException(language);

            StringBuilder regex = new StringBuilder();
            for (String contraction : IOUtils.readLines(stream, UTF8Charset.get())) {
              String prefix;
              if (contraction.charAt(0) != '\'') prefix = "[^\\p{L}]";
              else prefix = "\\p{L}";

              String suffix;
              if (contraction.charAt(contraction.length() - 1) != '\'') suffix = "[^\\p{L}]";
              else suffix = "\\p{L}";
            ...
            return Pattern.compile(
                regex.substring(0, regex.length() - 1), Pattern.CASE_INSENSITIVE);
          } catch (IOException e) {
