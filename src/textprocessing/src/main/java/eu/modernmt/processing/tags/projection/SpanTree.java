...
class SpanTree {
  ...
  public static class Node implements Comparable<Node> {
    ...
    void setParent(Node parent) {
      this.parent = parent;
      if (parent != null) this.getData().setLevel(parent.getData().getLevel() + 1);
    }
    ...
    void removeChild(Node child) {
      // the child remove isolated; its parent is set to null, but it is not added to the children
      // of the root
    ...
    Node clone(SpanCollection targetSpans) {
      ...
      this.children.stream()
          .map(sourceChild -> sourceChild.clone(targetSpans))
          .forEach(cloned::addChild);
      return cloned;
    }
  }
  ...
  protected Node create(int spanIdx, List<Integer> spanVisit) {
    ...
    // search for the first span on the right which has level = root.getLevel()+1, but is not child
    // of a sibling
    while (firstChildIdx < spans.size()) {
      if ((spans.get(firstChildIdx).getLevel() == rootLevel)
          || (spans.get(firstChildIdx).getLevel() == rootLevel + 1)) {
        break;
      }
      firstChildIdx++;
    }
  ...
  private static void fixAnchors(Node node, SpanCollection sourceSpans) {
    Span targetSpan = node.getData();
    targetSpan.setAnchor(computeAnchor(node, sourceSpans, true));

    for (Node child : node.getChildren()) fixAnchors(child, sourceSpans);
  }

  private static int computeAnchor(Node node, SpanCollection sourceSpans, boolean move) {
    ...
    if (span.getBeginTag() == null) {
      ...
      if (targetAnchor == -1 || node.getData().getPositions().isEmpty()) {
        ...
        if (parentPositions.size() == 0) {
          ...
          if (targetAnchor == -1) {
            float ratio =
                (float) (sourceAnchor - sourceParentPositions.getMin())
                    / sourceParentPositions.size();
            targetAnchor = Math.round(parentPositions.getMin() + parentPositions.size() * ratio);
          }

          if (move) {
            // check if the guessed targetAnchor falls into a sibling spanning at least two
            // positions
            // not in the first position; if it falls in the first position is means that it is
            // position outside (on left)
  ...
  private static void fixChildrenPositions(Node parent) {
  ...
  private static void fixChildrenPositions(Node parent, Node child) {
    Coverage positions =
        Coverage.difference(child.getData().getPositions(), parent.getData().getPositions());
  ...
  private static boolean isArtificial(Node node) {
    return (node.getData().getBeginTag() == null) || (node.getData().getEndTag() == null);
  }

  private static void fixTwoArtificialSiblings(Node childI, Node childJ) {
    if (isArtificial(childI) && isArtificial(childJ)) fixSiblings(childI, childJ);
  }

  private static Node fixOneArtificialSiblings(
      Node childI, Node childJ, SpanCollection sourceSpans) {
    if ((isArtificial(childI) && isArtificial(childJ))
        || (!isArtificial(childI) && !isArtificial(childJ))) return null;
    ...
    if (targetAnchor == -1) childSpan.setAnchor(computeAnchor(currentChild, sourceSpans, false));

    if ((!positions.isEmpty() && Coverage.difference(childPos, positions).isEmpty())
        || (childPos.isEmpty()
            && parentPos.contains(targetAnchor)
            && targetAnchor > parentPos.getMin())) {
      ...
      for (Node child : currentChild.getChildren()) futureNode.addChild(child);
  ...
  private static void fixStandardSiblings(Node childI, Node childJ) {
    if (!isArtificial(childI) && !isArtificial(childI)) fixSiblings(childI, childJ);
  }

  private static void fixSiblings(Node childI, Node childJ) {
    ...
    while (modified) {
      ...
      Coverage positions =
          Coverage.intersection(Coverage.contiguous(posI), Coverage.contiguous(posJ));
  ...
  private static void fixNode(Node node, Set<Node> nodeVisit, SpanCollection sourceSpans) {
      ...
      if (node.getChildren().size() > 1) { // there are at least two children
        ...
        while (iteratorI.hasNext()) {
          ...
          for (Node value : node.getChildren()) {
            ...
            if (childI.getId() >= childJ.getId()
                || nodeVisit.contains(childJ)
                || childrenToRemove.contains(childJ))
              // childI and childJ are already considered or childJ is already fixed ir childJ has
              // to be removed from the chilren
              continue;

            Node childToRemove = fixOneArtificialSiblings(childI, childJ, sourceSpans);
            if (childToRemove != null) childrenToRemove.add(childToRemove);
          }
        }
