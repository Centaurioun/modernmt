...
class SpanCollection implements Iterable<Span> {
  ...
  SpanCollection(List<Tag> tags, int words) {
    ...
    for (String name : openingTagSet.keySet()) {
      ...
      if (closingTags == null) {
        // there are no closing tags for this name; hence all opening tags for this name are
        // spurious
    ...
    for (String name : closingTagSet.keySet()) {
      ...
      List<Integer> closingTags = closingTagSet.get(name);
      if (openingTags == null) {
        // there are no opening tags for this name; hence all closing tags for this name are
        // spurious
    ...
    for (int t = 0; t < tags.size(); t++) {
      ...
      if (tagLink[t] != -1) {
        ...
        if (type == Tag.Type.OPENING_TAG) {
          ...
          // A SEPARATOR_TAG becomes a span with a beginTag equals to "null"; this means that it
          // covers all left positions
          beginTag = null;
          endTag = tag;
        }
      }
    ...
    for (Span sourceSpan : sourceSpans) {
      Span targetSpan =
          new Span(
              sourceSpan.getId(),
              sourceSpan.getLevel(),
              sourceSpan.getBeginTag(),
              sourceSpan.getEndTag(),
              targetWords);
