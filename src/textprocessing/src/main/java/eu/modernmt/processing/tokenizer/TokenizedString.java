...
public class TokenizedString {
    ...
    for (char c : source) {
      ...
      switch (type) {
        ...
        default:
          ...
          if (Character.isSurrogatePair(previousChar, c)) this.flags[index] |= PROTECTED_FLAG;
  ...
  public int protect(int start, int end) {
    ...
    for (int i = start; i < end; i++) {
      ...
      if (((flag & SPLIT_FLAG) > 0) && ((flag & PROTECTED_FLAG) == 0)) index = i;
  ...
  public SentenceBuilder compile(SentenceBuilder builder) {
    ...
    for (int i = 0; i < length + 1; i++) {
      ...
      if (i < chars.length) {
        if (chars[i] == ' ') {
          if (!foundNonWhitespace) tokenStart++;
        } else {
