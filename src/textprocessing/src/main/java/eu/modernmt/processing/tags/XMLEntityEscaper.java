 ...
 *
 * <p>An XMLEntityEscaper is the object that, in the processing pipeline, handles the identification
 * of XML Escape Entities and requests to the StringBuider editor their replacement with the
 * character they stand for.
 */
public class XMLEntityEscaper extends TextProcessor<SentenceBuilder, SentenceBuilder> {

  /**
   * This method uses a Matcher and the pattern described by XMLCharEntity to find all XML Escape
   * Entities in the input SentenceBuilder current String. For each Entity found, it then proceeds
   * to ask the XMLCharacterEntity what character char it stands for, and requests the
   * SentenceBuilder editor to set a Replacement in order to substitute its text with char.
   *
  ...
  public SentenceBuilder call(SentenceBuilder builder, Map<String, Object> metadata) {
    ...
    while (m.find()) {
      ...
      if (c != null) editor.replace(start, end - start, c);
    }
    editor.commit();
    return builder;
  }
