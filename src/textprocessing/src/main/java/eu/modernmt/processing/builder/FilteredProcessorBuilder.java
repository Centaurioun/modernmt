...
class FilteredProcessorBuilder extends ProcessorBuilder {
  ...
  private static Filter parseFilter(String definition) {
    ...
    if (definition.length() < 2) return null;

    if (definition.charAt(0) == '^')
      return new NorFilter(definition.substring(1).trim().split("\\s+"));
    else return new OrFilter(definition.split("\\s+"));
  }

  public boolean accept(Language sourceLanguage, Language targetLanguage) {
    if (this.sourceFilter != null && !this.sourceFilter.accept(sourceLanguage)) return false;
    if (this.targetFilter != null && !this.targetFilter.accept(targetLanguage)) return false;
    return true;
  }
      ...
      for (String lang : languages) {
        ...
        if (!parsed.getLanguage().equals(parsed.toLanguageTag()))
          throw new IllegalArgumentException(
              "Text processing framework only supports language-only tags, but found complex"
                  + " language: "
                  + lang);
