    ...
    if (source.hasTags()) {
      ...
      if (source.hasWords()) {
        if (translation.hasAlignment()) {
          ...
          Alignment alignment =
              new Alignment(
                  translation.getWordAlignment(), sourceWords.length, translationWords.length);
  ...
  public static void simpleSpaceAnalysis(Sentence sentence) {
    ...
    for (Token currentToken : sentence) {
      ...
      if (previousToken == null) {
        ...
        if (currentToken instanceof Tag) { // X-Tag
          ...
          if (previousToken instanceof Tag) { // Tag-Word
            if (((Tag) previousToken).getType() != Tag.Type.SEPARATOR_TAG) {
              ...
              // but no Token between the last Word and this Word has any space (ex.
              // "previousWord<tag1><tag2>thisWord");
              // hence force a space before this Word
              if (spaceAfterPreviousWord == null
                  && ((Word) currentToken).isLeftSpaceRequired()
                  && !((Word) currentToken).hasHiddenLeftSpace()) {
        ...
        if (!(currentToken instanceof Tag)) wordIdx++;
        if (wordIdx == wordN - 1) lastWord = true;
      }
