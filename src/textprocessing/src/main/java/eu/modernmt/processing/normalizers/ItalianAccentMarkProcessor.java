...
public class ItalianAccentMarkProcessor extends TextProcessor<SentenceBuilder, SentenceBuilder> {
  ...
  private static final Pattern COMMON =
      Pattern.compile(
          "(((^|\\W)e)|\\w+[a-z][r][ao]|\\w+[gnr]no|\\w+io|\\w+t[aeiou]|(cio|cosi|gia|laggiu|lassu|li|la|menu|piu|puo|pero|quaggiu|quassu|si|lunedi|martedi|mercoledi|giovedi|venerdi))'",
          Pattern.CASE_INSENSITIVE);
  private static final Pattern SPECIAL_WORDS =
      Pattern.compile(
          "(^|\\W)(affinch|alch|alcunch|allorch|almenoch|altroch|amenoch|ammenoch|ancorch|anzicch|anzich|"
              ...
              + "|pressocch|pressoch|semprech|sennonch|senonch|sicch|n|\\w+tr)(e'|è)",
          Pattern.CASE_INSENSITIVE);

  public ItalianAccentMarkProcessor(Language sourceLanguage, Language targetLanguage)
  ...
  private static SentenceBuilder replaceCommonWords(SentenceBuilder builder) {
    ...
    for (int i = 0; i < array.length; i++) {
      if (array[i] == '\'') {
        ...
        if (leftWhitespace) {
          if (rightWhitespace) {
            if (pendingApostrophesCount > 0) pendingApostrophesCount--;
