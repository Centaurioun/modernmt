...
public class NumericWordPostprocessor extends TextProcessor<Translation, Translation> {
  ...
  public Translation call(Translation translation, Map<String, Object> metadata) {
    ...
    for (NumericPlaceholder placeholder : nTranslation) placeholder.obfuscate();

    return translation;
  }

  private static void tryWithPhrases(
    ...
    for (Phrase phrase : phrases) {
      ...
      if (sourceDigitsCount == targetDigitsCount) phrase.copySourceToTargetDigits();
      else phrase.copySourceToTargetWords();
    }
  }

  private static void tryWithPositionAndPlaceholderDigits(
      NumericSequence sourceSequence, NumericSequence targetSequence) {
    if (targetSequence.isEmpty()) return;

    HashMap<Integer, LinkedList<NumericPlaceholder>> digits2placeholder = new HashMap<>();
    for (NumericPlaceholder e : sourceSequence) {
      LinkedList<NumericPlaceholder> list =
          digits2placeholder.computeIfAbsent(e.getDigits().length, (key) -> new LinkedList<>());
      list.add(e);
    }
  ...
  private static void tryWithPlaceholderPosition(
      NumericSequence sourceSequence, NumericSequence targetSequence) {
    if (targetSequence.isEmpty()) return;
