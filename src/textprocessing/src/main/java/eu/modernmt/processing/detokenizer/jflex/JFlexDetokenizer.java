...
public class JFlexDetokenizer extends TextProcessor<Translation, Translation> {
  ...
  private static final Map<String, Class<? extends JFlexSpaceAnnotator>> ANNOTATORS =
      new HashMap<>();
  ...
  public Translation call(Translation translation, Map<String, Object> metadata) {
    ...
    text.applyLeft(
        ...
        (word, hasSpace) -> {
          ...
          if (word.hasLeftSpace()) word.setLeftSpace(hasSpace ? " " : null);
        });
    ...
    text.applyRight(
        ...
        (word, hasSpace) -> {
          ...
          if (word.hasRightSpace()) word.setRightSpace(hasSpace ? " " : null);
        });
