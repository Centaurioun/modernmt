 ...
 *
 * <p>An IndexMap object maps the position of each character in the SentenceBuilder current String
 * to the positions of the corresponding character in the SentenceBuilder original string.
 *
 * <p>The IndexMap thus stores information from all the editing activities committed so far on the
 * current String. When the SentenceBuilder starts working on a new string, the IndexMap is just
 * re-initialized instead of creating a new one, in order to save memory and time: a SentenceBuilder
 * uses one and only IndexMap object during its whole lifecycle.
 */
public class IndexMap {
  ...
  /**
   * This constructor generates an empty IndexMap object. The positions array is set to null, and
   * its length to 0.
   *
   * <p>This constructor should only be called during the creation of the SentenceBuilder, before
   * the processing of any string.
   */
  public IndexMap() {
    this.positions = null;
    this.arrayLength = 0;
  ...
  /** This constructor generates an IndexMap object based on the length of the string to process. */
  ...
  /**
   * This method initializes an IndexMap object by setting its length and populating the positions
   * array, on the basis of the length of the string to edit.
   *
   * <p>The positions array length must be equal to the string length + 1 (this lets us use the
   * substring method on the last part of the string too). Therefore, if the current version of the
   * positions array is too small for the current string, it is replaced by a larger array. Anyway
   * its content is set so that for each i, positions[i] holds i itself.
   *
   * <p>A new IndexMap, in fact, since no edits have been committed yet, just maps the content of
   * the SentenceBuilder original String with itself. (because the original string is the first
   * version of the currentString).
   *
  ...
  /**
   * This method transforms the current positions array by taking its portion between start
   * (included) and end (not included) and making it become as long as newLength says. It is only
   * invoked when a replacement is performed by the Editor.
   *
   * <p>Note: at the moment, only non-void replacements are allowed (you can not replace something
   * with "") except when the void replacement takes places at the start or at the end of the string
   *
  ...
  public void update(int start, int end, int newLength) {
    ...
    if (newLength > oldLength) {
      ...
      for (int i = 1; i < newLength; i++) {
        this.positions[start + i] =
            (int)
                Math.round(
                    this.positions[start]
                        + ((this.positions[start + i] - positions[start]) * ratio));
      }
    }
  ...
  /**
   * This method selects a portion of the array and and shifts it backwards or forwards for a
   ...
   * @param shiftStep the amount of positions that the portion must be shifted of. if it is greater
   *     than 0, the shift must be shifted forwards; if it is smaller than 0 the portion must be
   *     shifted backwards.
   */
