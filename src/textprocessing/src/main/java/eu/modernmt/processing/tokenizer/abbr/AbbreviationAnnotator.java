...
public class AbbreviationAnnotator implements BaseTokenizer.Annotator {
  ...
  private static final ConcurrentHashMap<String, AbbreviationAnnotator> instances =
  ...
  private static HashMap<String, Boolean> readResource(String resourceName, boolean caseless)
      throws IOException {
    UnixLineReader reader = null;

    try {
      InputStream stream = AbbreviationAnnotator.class.getResourceAsStream(resourceName);
      if (stream == null) throw new IOException("Abbreviation resource not found: " + resourceName);
      ...
      if (words.isEmpty()) throw new IOException("File does not contain valid abbreviations");
  ...
  private static char[] getAlphabet(Set<String> words, boolean caseless) {
    ...
    for (String word : words) {
      if (caseless) word = word.toLowerCase();
  ...
  private boolean contains(char[] chars, char c) {
    if (this.caseless) c = Character.toLowerCase(c);
  ...
  private int getLongestMatch(String chars, int end) {
    ...
    for (int i = end - 1; i >= left; i--) {
      ...
      if (contains(SEPARATORS, c)) {
        ...
        if (numberOnly != null && (!numberOnly || digitFollows)) result = i + 1;
      }
