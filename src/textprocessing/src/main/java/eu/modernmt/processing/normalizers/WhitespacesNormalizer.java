...
/**
 * Created by davide on 19/02/16. Updated by andrearossi on 01/03/17
 *
 * <p>A RareCharsNormalizer has the responsibility to know what are the whitespace characters to
 * take into account, to know how to process them (delete them or replace them) case by case to look
 * for whitespace characters sequences in toe string under analysis and to actively request the
 * specific processing they need.
 */
public class WhitespacesNormalizer extends TextProcessor<SentenceBuilder, SentenceBuilder> {
   ...
   * looking for whitespace characters sequences and requests either their deletion, if they are at
   * the very beginning or at the very end end of the string or their their replacement with a blank
   * space (" ") in any other case.
   *
  ...
  public SentenceBuilder call(SentenceBuilder builder, Map<String, Object> metadata) {
    ...
    for (i = 0; i < source.length; i++) {
      ...
      if (isWhitespace(c)) {
        ...
        if (whitespaceStart >= 0) {
          ...
          if (sentenceBegin) editor.delete(whitespaceStart, i - whitespaceStart);
          /*else, it the editor must replace it with a blank space (" ")*/
          else editor.replace(whitespaceStart, i - whitespaceStart, " ");
    ...
    if (whitespaceStart >= 0) editor.delete(whitespaceStart, i - whitespaceStart);
