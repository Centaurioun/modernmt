...
public class ChineseCharacterConverter extends TextProcessor<Translation, Translation> {
  ...
  private static class DictionaryLoadException extends ProcessingException {
  ...
  public ChineseCharacterConverter(Language sourceLanguage, Language targetLanguage)
      throws UnsupportedLanguageException {
    super(sourceLanguage, targetLanguage);
    String language = targetLanguage.getLanguage();
    String region = targetLanguage.getRegion();
    if (!"zh".equals(language)) throw new UnsupportedLanguageException(targetLanguage);
    if (region == null) chars = null;
    else if ("CN".equals(region)) chars = loadDictionary("TSCharacters.txt");
    else if ("TW".equals(region)) chars = loadDictionary("STCharacters.txt");
    else throw new UnsupportedLanguageException(targetLanguage);
  }
  ...
  public Translation call(Translation param, Map<String, Object> metadata) {
    if (chars != null) {
      for (Word word : param.getWords()) {
        ...
        if (text != null) word.setText(text);
      }
    }
    return param;
  }
  ...
  private String convert(String text) {
    if (text.codePoints().noneMatch(chars::containsKey)) return null;
  ...
  private static Map<Integer, Integer> loadDictionary(String filename) {
    ...
    try {
      ...
      throw new DictionaryLoadException("Error loading dictionary: " + filename);
    } finally {
