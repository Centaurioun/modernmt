...
public class Phrase {
  ...
  public static List<Phrase> extract(
      Translation translation, NumericSequence sourceSequence, NumericSequence targetSequence) {
    if (!translation.hasAlignment()) return Collections.emptyList();
    ...
    for (int i = 0; i < sourceLength; i++) {
      if (coveredInSource.get(i)) continue;
  ...
  private static void cover(
      int word,
      boolean fromSource,
      BitSet coveredPerPhraseInSource,
      BitSet coveredPerPhraseInTarget,
      Alignment alignment) {
    if (fromSource) coveredPerPhraseInSource.set(word);
    else coveredPerPhraseInTarget.set(word);
  ...
  private static NumericPlaceholder[] extractPlaceholders(NumericSequence sequence, BitSet map) {
    ...
    for (int i = 0; i < map.length(); i++) {
      if (map.get(i) && sequence.hasIndex(i)) indexes[size++] = i;
    }

    NumericPlaceholder[] result = new NumericPlaceholder[size];
    for (int i = 0; i < size; i++) result[i] = sequence.getByIndex(indexes[i]);
  ...
  private Phrase(
      NumericSequence sourceSequence,
      NumericPlaceholder[] source,
      NumericSequence targetSequence,
      NumericPlaceholder[] target) {
  ...
  public int countSourceDigits() {
    int count = 0;
    for (NumericPlaceholder e : source) count += e.getDigits().length;
    return count;
  }

  public int countTargetDigits() {
    int count = 0;
    for (NumericPlaceholder e : target) count += e.getDigits().length;
    return count;
  }
  ...
  public void copySourceToTargetDigits() {
    ...
    for (NumericPlaceholder e : source) sourceSequence.remove(e);
    for (NumericPlaceholder e : target) targetSequence.remove(e);
  }
