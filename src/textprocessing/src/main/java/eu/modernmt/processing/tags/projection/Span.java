...
class Span implements Comparable<Span> {
  ...
  Span(int id, int level, Tag beginTag, Tag endTag, int words) {
    ...
    for (int i = begin; i < end; i++) this.positions.add(i);
  ...
  public int compareTo(Span a) {
    if (this.anchor == a.getAnchor()) return this.getPositions().size() - a.getPositions().size();
    else return this.anchor - a.getAnchor();
  }
}
