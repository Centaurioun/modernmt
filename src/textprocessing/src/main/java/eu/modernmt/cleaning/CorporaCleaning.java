...
public class CorporaCleaning {
  ...
  private static <T extends Composer> T compose(T composer, Options options) {
    ...
    if (options.eraseXml) composer.add(new DeepXMLEraser());
    else composer.add(new XMLStripper());
    ...
    if (options.filterBrokenUTF8) composer.add(BrokenUTF8Filter.class);
    if (options.filterByPunctuation) composer.add(PunctuationFilter.class);
    if (options.filterNumericSentences) composer.add(NumericTextFilter.class);
    if (options.filterVerbatimTranslations)
      composer.addMultilingual(VerbatimTranslationFilter.class);
    if (options.filterOddSentences) composer.add(RareNgramFilter.class);
    if (options.filterDrafts) composer.addMultilingual(DraftFilter.class);
    if (options.filterBySentenceLength) composer.addMultilingual(SentenceLengthFilter.class);
    if (options.filterByLanguageBatch) composer.add(OptimaizeLanguageBatchFilter.class);
    if (options.filterByLanguageInline) composer.add(OptimaizeLanguageFilter.class);
  ...
  private static class MultilingualComposer implements Composer {
    ...
    private final ChainedMultilingualCorpusFilter.Builder builder =
        new ChainedMultilingualCorpusFilter.Builder();
