...
abstract class AbstractOptimaizeFilter implements CorpusFilter {
  ...
  private static final HashSet<String> SUPPORTED_LANGUAGES =
      new HashSet<>(
          Arrays.asList(
              "af", "an", "ar", "ast", "be", "br", "ca", "bg", "bn", "cs", "cy", "da", "de", "el",
              "en", "es", "et", "eu", "fa", "fi", "fr", "ga", "gl", "gu", "he", "hi", "hr", "ht",
              "hu", "id", "is", "it", "ja", "km", "kn", "ko", "lt", "lv", "mk", "ml", "mr", "ms",
              "mt", "ne", "nl", "no", "oc", "pa", "pl", "pt", "ro", "ru", "sk", "sl", "so", "sq",
              "sr", "sv", "sw", "ta", "te", "th", "tl", "tr", "uk", "ur", "vi", "wa", "yi", "zh"));
  ...
  protected String guessLanguage(CharSequence text, boolean largeText, float minProbability) {
    ...
    if (largeText) {
      ...
      factory = new TextObjectFactoryBuilder().withTextFilter(UrlTextFilter.getInstance()).build();
    }
    ...
    if (lang.getProbability() < minProbability && languages.size() > 1) return null;
