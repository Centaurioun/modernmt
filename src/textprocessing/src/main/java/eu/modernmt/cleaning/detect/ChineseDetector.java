...
/**
 * ChineseDetector holds the vocabularies of Simplified/Traditional Chinese and variants for
 * detecting the actual language of a text
 */
public class ChineseDetector {
  ...
  public static ChineseDetector getInstance() {
    if (instance == null) {
      synchronized (ChineseDetector.class) {
        if (instance == null) instance = new ChineseDetector();
      }
    }
  ...
  private HashSet<Integer> loadDict(String region) {
    ...
    try {
      ...
      if (url == null) throw new IOException("Vocabulary " + filename + " does not exist.");
  ...
  public Language detect(String string) {
    ...
    if (simplifiedCount == traditionalCount) return Language.CHINESE;
    else if (simplifiedCount > traditionalCount) return Language.CHINESE_SIMPLIFIED;
