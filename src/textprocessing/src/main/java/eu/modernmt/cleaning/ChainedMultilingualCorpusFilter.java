...
public class ChainedMultilingualCorpusFilter implements MultilingualCorpusFilter, CorpusNormalizer {
  ...
  private ChainedMultilingualCorpusFilter(
      CorpusNormalizer[] normalizers, MultilingualCorpusFilter[] filters) {
  ...
  public String normalize(String line) {
    for (CorpusNormalizer normalizer : normalizers) line = normalizer.normalize(line);
    return line;
  }
  ...
  public Initializer getInitializer() {
    ...
    if (initializers.isEmpty()) {
      ...
      return new Initializer() {
        @Override
        public void onBegin() {
          for (Initializer initializer : initializers) initializer.onBegin();
        }

        @Override
        public void onTranslationUnit(TranslationUnit tu, int index) {
          for (Initializer initializer : initializers) initializer.onTranslationUnit(tu, index);
        }

        @Override
        public void onEnd() {
          for (Initializer initializer : initializers) initializer.onEnd();
        }
      };
    }
  }
  ...
  public boolean accept(TranslationUnit tu, int index) {
    for (MultilingualCorpusFilter filter : filters) {
      if (!filter.accept(tu, index)) return false;
    }
  ...
  @Override
  public void clear() {
    for (MultilingualCorpusFilter filter : filters) filter.clear();
  }
}
