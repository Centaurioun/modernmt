...
public class DraftFilter implements MultilingualCorpusFilter {
  ...
  public Initializer getInitializer() {
    ...
    return new Initializer() {
      ...
      public void onTranslationUnit(TranslationUnit tu, int index) {
        ...
        if (timestamp == null) timestamp = new Date(lastTimestamp.getTime() + 60L * 1000L);
        lastTimestamp = timestamp;

        filters
            .computeIfAbsent(tu.language, k -> new DraftFilterData())
      ...
      public void onEnd() {
        ...
        for (DraftFilterData filter : filters.values()) filter.compile();
      }
    };
  }
  ...
  @Override
  public void clear() {
    for (DraftFilterData filter : filters.values()) filter.clear();
  }
}
