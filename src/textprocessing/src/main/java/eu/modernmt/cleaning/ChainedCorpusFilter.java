...
public class ChainedCorpusFilter implements CorpusFilter, CorpusNormalizer {
  ...
  public Initializer getInitializer() {
    ...
    if (initializers.isEmpty()) {
      ...
      return new Initializer() {
        @Override
        public void onBegin() {
          for (CorpusFilter.Initializer initializer : initializers) initializer.onBegin();
        }
        ...
        @Override
        public void onEnd() {
          for (CorpusFilter.Initializer initializer : initializers) initializer.onEnd();
        }
      };
    }
  }
  ...
  public boolean accept(Language language, String line, int index) {
    for (CorpusFilter filter : filters) {
      if (!filter.accept(language, line, index)) return false;
    }
  ...
  @Override
  public void clear() {
    for (CorpusFilter filter : filters) filter.clear();
  }
}
