...
public class XMLStripper implements CorpusNormalizer {
  ...
  public static final Pattern DoubleEntityPattern =
      Pattern.compile("&amp;((#[0-9]{1,4})|(#x[0-9a-fA-F]{1,4})|([a-zA-Z]+));");
  ...
  private static String strip(String line) {
    while (true) {
      // if it has valid XML then we stop: if you put actual XML all subsequent entities should be
      // intentional
      String plain = stripXML(line);
      if (!plain.equals(line)) return plain;

      // if there are no double-escaped entities (i.e. &amp;lt;), problem is solved!
      String stripped = stripDoubleEscapedEntities(line);
      if (stripped.equals(line)) return line;

      // if, excluding double-escaped entities, we found entities, it means that the first ones were
      // intentional
      if (hasXMLEntities(stripped)) return line;
