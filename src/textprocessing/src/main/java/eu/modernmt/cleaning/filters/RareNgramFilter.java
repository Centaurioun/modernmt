...
public class RareNgramFilter implements CorpusFilter {
  ...
  public Initializer getInitializer() {
    return new Initializer() {
      ...
      public void onLine(Language language, String line, int index) {
        ...
        if (line.length() < MIN_SENTENCE_LENGTH) return;
      ...
      private HashSet<String> filterCounts(HashMap<String, Counter> vocabulary, double threshold) {
        ...
        double size = 0;
        for (Entry e : entries) size += e.count;
        ...
        for (Entry e : entries) {
          ...
          if ((accumulator / size) >= threshold) break;
        }
  ...
  private static String[] tokenize(String string) {
    ...
    if (length < 3) return new String[0];
    ...
    if (size > 0) {
      for (int i = 0; i < size; i++) result[i] = string.substring(i, i + 3);
    }
  ...
  public boolean accept(Language language, String line, int index) {
    ...
    if (line.length() < MIN_SENTENCE_LENGTH) return true;
  ...
  private double match(String line) {
    ...
    for (String token : tokenize(line)) {
      length++;
      if (words.contains(token)) matches++;
    }
