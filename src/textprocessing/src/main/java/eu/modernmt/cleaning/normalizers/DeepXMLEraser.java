...
public class DeepXMLEraser implements CorpusNormalizer {
  ...
  private static boolean isBlacklisted(XMLTag tag) {
    if (tag.isComment() || tag.isDTD() || !tag.isOpeningTag()) return true;
  ...
  private static String stripXML(String line) {
    ...
    while (m.find()) {
      ...
      if (stringIndex < mstart) builder.append(chars, stringIndex, mstart - stringIndex);

      XMLTag tag = XMLTag.fromText(m.group());
      if (isBlacklisted(tag)) builder.append(' ');
    ...
    if (stringIndex < chars.length) builder.append(chars, stringIndex, chars.length - stringIndex);
  ...
  public String normalize(String line) {
    while (true) {
      ...
      if (unescaped.equals(xmlStrip)) return xmlStrip;
