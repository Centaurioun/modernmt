...
/**
 * Created by andrea on 29/09/17. A MySQLImportJobDAO object offers methods for performing CRUD
 * operations on ImportJob objects when connected to a MySQL Database.
 */
public class MySQLImportJobDAO implements ImportJobDAO {
  ...
  /**
   * Create a MySQLImportJobDAO that will communicate with the MySQLDatabase using a specific
   * connection
   *
   ...
   * @return if the domain was successfully stored, the method returns the ResourceOrigin itself
   *     (with its ID updated to the new one) Else, it throws an exception.
  ...
  public ImportJob store(ImportJob importJob) throws PersistenceException {
    String query =
        "INSERT INTO mmt_import_jobs(memory, begin, end, data_channel, size) VALUES (?,?,?,?,?)";
  ...
  /**
   * This method reads the fields of a ResultSet from a table with name "resource_origins" and
   * creates a new ResourceOrigin object
   *
  ...
  /**
   * This method reads the fields of a ResultSet from the table with given name and creates a new
   * ImportJob object
   *
   * @param result a resultSet obtained by a query
