...
public class PreprocessorMain {
  ...
  private static class Args {
    ...
    static {
      ...
      Option skipPlaceholders =
          Option.builder().longOpt("print-placeholders").hasArg(false).required(false).build();
      Option keepSpaces =
          Option.builder().longOpt("original-spacing").hasArg(false).required(false).build();
      Option sentenceSplit =
          Option.builder().longOpt("sentence-split").hasArg(false).required(false).build();
    ...
    public Args(String[] args) throws ParseException {
      ...
      configFile =
          cli.hasOption("preprocessor") ? new File(cli.getOptionValue("preprocessor")) : null;
    }
  }
  ...
  public static void main(String[] _args) throws Throwable {
    ...
    try {
      if (args.configFile != null) {
        XMLPipelineBuilder<String, Sentence> builder =
            XMLPipelineBuilder.loadFromXML(args.configFile);
        preprocessor = new Preprocessor(builder);
      } else {
        preprocessor = new Preprocessor();
      }

      if (args.keepSpaces) output = new SentenceOutputter(args.printTags, args.printPlaceholders);
      else output = new TokensOutputter(args.printTags, args.printPlaceholders);

      if (args.batch) batchPreprocess(preprocessor, args.language, input, output, args.split);
      else interactivePreprocess(preprocessor, args.language, input, output, args.split);
  ...
  private static void interactivePreprocess(
  ...
  private static void batchPreprocess(
      Preprocessor preprocessor,
      LanguageDirection language,
      LineReader input,
      Outputter output,
    ...
    while ((batch = bufferedReader.readLines()) != null) {
      ...
      for (Sentence sentence : tokenized) write(output, sentence, split);
    }
  }

  private static void write(Outputter outputter, Sentence sentence, boolean split)
      throws IOException {
    List<Sentence> sentences = null;

    if (split) sentences = SentenceSplitter.split(sentence);

    if (sentences == null || sentences.size() < 2) {
      outputter.write(sentence);
    } else {
      for (Sentence sentenceSplit : sentences) outputter.write(sentenceSplit);
    }
  }
