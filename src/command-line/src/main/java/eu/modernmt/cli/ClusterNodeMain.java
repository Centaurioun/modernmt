...
/** Created by davide on 22/04/16. Updated by andrearossi on 18/04/16. */
public class ClusterNodeMain {
  ...
  /**
   * This class models the list of arguments that can be passed to the ClusterNodeMain main. It is
   * typically created by parsing the arguments passed from the outside during the queue of MMT
   * start.
   */
  private static class Args {
    ...
    /**
     * This builder parses the command line arguments passed by the Python module into the Options
     * object statically defined by Args. After that, it reads the various options from the Options
     * object and stores them in the corresponding instance variables.
     *
     * <p>Finally, it uses the obtained options to create a new NodeConfig. The Nodeconfig is
     * initially generated by parsing the engine.xconf file, and then its values are overwritten if
     * necessary with the cli values
     *
     * @param args the arguments passed by the Python module when invoking the ClusterNodeMain main
    ...
    public Args(String[] args) throws ParseException, ConfigException {
      ...
      if (port != null) netConfig.setPort(Integer.parseInt(port));
      String apiPort = cli.getOptionValue("api-port");
      if (apiPort != null) apiConfig.setPort(Integer.parseInt(apiPort));
      String binlogPort = cli.getOptionValue("binlog-port");
      if (binlogPort != null) binlogConfig.setPort(Integer.parseInt(binlogPort));
      String databasePort = cli.getOptionValue("db-port");
      if (databasePort != null) dbConfig.setPort(Integer.parseInt(databasePort));
  ...
  public static void main(String[] _args) throws Throwable {
    ...
    // Init logging
    if (args.logFile != null) Log4jConfiguration.setup(args.logFile, args.verbosity);
    else Log4jConfiguration.setup(args.verbosity);
  ...
  /**
   * A FileStatusListener subscribes to a ClusterNode and, when its properties change, updates the
   * properties file
   */
  public static class FileStatusListener implements ClusterNode.StatusListener {
    ...
    /**
     * This constructor employs the Nodeconfig resulting from the read of the engine.xconf file. and
     * uses it to initialize the JSON structures that will be later written in the node.properties
     * file.
     *
     * <p>- cluster port - api config (root and port), - database (host and port) - binlog (host and
     * port)
     *
     * @param file the node.properties JSON file in which to store the definitive configuration
    ...
    public FileStatusListener(File file, NodeConfig config) {
      ...
      if (apiConfig.isEnabled()) {
        ...
        if (root != null) api.addProperty("root", root);
    ...
    /**
     * Subscriber method: when the target ClusterNode changes its status, the embedded services and
     * the status in the properties file must be updated
     *
    ...
    public void onStatusChanged(
        ClusterNode node, ClusterNode.Status currentStatus, ClusterNode.Status previousStatus) {
      this.updateServices(node.getServices());

      if (currentStatus == ClusterNode.Status.RUNNING) return; // Wait for REST Api to be ready
