...
public class Log4jConfiguration {
  ...
  private static final Level[] VERBOSITY_LEVELS =
      new Level[] {Level.ERROR, Level.INFO, Level.DEBUG, Level.ALL};
  ...
  private static String loadConfig(Level level, File logFile) throws IOException {
    ...
    try {
      templateStream =
          Log4jConfiguration.class.getResourceAsStream(
    ...
    if (logFile != null) template = template.replace("%{log_file}", logFile.getAbsolutePath());
