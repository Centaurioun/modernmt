...
public class ConvertMain {
  ...
  private static class Args {
    ...
    public Args(String[] args) throws ParseException {
      ...
      inputFormat =
          cli.hasOption("input-format")
              ? FileFormat.fromName(cli.getOptionValue("input-format"))
              : null;
  ...
  public static void main(String[] _args) throws Throwable {
    ...
    if (args.input.isFile()) {
      MultilingualCorpus input =
          new MaskedMultilingualCorpus(
              args.language, args.inputFormat.parse(args.language, args.input));
      MultilingualCorpus output = args.outputFormat.rename(args.language, input, args.output);

      Corpora.copy(input, output);
    } else {
      BatchCopyProcess copy =
          new BatchCopyProcess(
              new BatchCopyProcess.OutputCorpusFactory() {
                @Override
