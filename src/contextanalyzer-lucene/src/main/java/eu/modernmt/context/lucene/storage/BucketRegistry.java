...
public class BucketRegistry implements Closeable {
  ...
  public synchronized Bucket get(long id, LanguageDirection language, UUID owner)
      throws IOException {
      ...
      return cache.computeIfAbsent(
          ...
          arg -> {
            try {
              ...
              return bucket == null
                  ? new Bucket(getBucketFolder(this.root, id), arg.id, arg.language, owner)
                  : bucket;
            } catch (IOException e) {
    ...
    try {
      statement =
          connection.prepareStatement(
              "SELECT owner_lsb, owner_msb, size, plain_size, gz_size "
      ...
      if (result.next()) {
        ...
        return new Bucket(
            getBucketFolder(this.root, id), id, language, owner, plainSize, gzSize, size);
      } else {
        return null;
      }
    } catch (SQLException e) {
    ...
    try {
      ...
      result =
          statement.executeQuery(
              "SELECT id, source, target, owner_lsb, owner_msb, size, plain_size, gz_size FROM"
                  + " buckets WHERE id = "
                  + id);

      while (result.next()) set.add(parseBucket(result));
  ...
  public synchronized void mark(Bucket bucket, long mark) throws IOException {
    ...
    try {
      statement =
          connection.prepareStatement(
              "UPDATE buckets SET mark = ? WHERE id = ? AND source = ? AND target = ?");
  ...
  public synchronized void update(Map<Short, Long> channels, Set<Bucket> buckets)
      throws IOException {
    ...
    try {
      ...
      channelStatement =
          connection.prepareStatement(
              "INSERT OR REPLACE INTO channels(id, position) VALUES (?, ?)");
      iBucketStatement =
          connection.prepareStatement(
              "INSERT INTO buckets(id, source, target, owner_lsb, owner_msb, size, plain_size,"
                  + " gz_size) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
      uBucketStatement =
          connection.prepareStatement(
              "UPDATE buckets SET size = ?, plain_size = ?, gz_size = ? WHERE id = ? AND source = ?"
                  + " AND target = ?");
  ...
  private static void finalizeTransaction(Connection connection, boolean success)
      throws IOException {
    try {
  ...
  private static UUID getUUID(ResultSet result, int lsbIndex, int msbIndex) throws SQLException {
    ...
    UUID uuid = null;
    if (lsb > 0 || msb > 0) uuid = new UUID(msb, lsb);
    return uuid;
  }
  ...
  public synchronized void close() throws IOException {
    try {
      for (Bucket bucket : cache.values()) bucket.getWriter().close();
    ...
    public CacheKey(long id, LanguageDirection language, boolean maskLanguageRegion) {
      if (maskLanguageRegion) {
        ...
        if (owSource != null || owTarget != null) {
          if (owSource == null) owSource = language.source;
          if (owTarget == null) owTarget = language.target;
