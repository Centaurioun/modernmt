...
public class Dump {
  ...
  private static final class DumpableCorporaStorage extends CorporaStorage {
    ...
    public void forEach(Consumer<Bucket> consumer) throws IOException {
      for (Bucket bucket : super.buckets.getAll()) consumer.accept(bucket);
    }
  }
  ...
  private static void dump(Bucket bucket) throws IOException {
    String prefix =
        bucket.getId()
            + "\t"
            + bucket.getLanguage().source
            + "\t"
            + bucket.getLanguage().target
            + "\t";
    ...
    try {
      ...
      String line;
      while ((line = reader.readLine()) != null) System.out.println(prefix + line);
    } finally {
