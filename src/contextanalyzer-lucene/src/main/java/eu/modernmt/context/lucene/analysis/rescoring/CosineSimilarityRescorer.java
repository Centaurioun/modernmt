...
public class CosineSimilarityRescorer implements Rescorer {
  ...
  public void rescore(
    ...
    Map<String, Float> referenceTerms =
        LuceneUtils.getTermFrequencies(analyzer, reference, fieldName);
    ...
    try {
      ...
      for (int i = 0; i < tasks.length; i++)
        tasks[i] =
            executor.submit(
                new RescoringTask(reader, fieldName, topDocs[i], referenceTerms, referenceL2Norm));
  ...
  public static double getL2Norm(Map<String, Float> terms) throws IOException {
    ...
    for (Float value : terms.values()) norm += value * value;
  ...
  private static class RescoringTask implements Callable<Void> {
    ...
    public RescoringTask(
        IndexReader reader,
        String fieldName,
        ScoreDoc target,
        Map<String, Float> referenceTerms,
        double referenceL2Norm) {
      this.reader = reader;
    ...
    public Void call() throws IOException {
      Map<String, Float> terms =
          LuceneUtils.getTermFrequencies(this.reader, this.target.doc, this.fieldName);

      double dotProduct = 0;
      double l2Norm = 0;

      for (Float value : terms.values()) l2Norm += value * value;
      l2Norm = Math.sqrt(l2Norm);

      for (Map.Entry<String, Float> entry : referenceTerms.entrySet()) {
        Float otherFreq = terms.get(entry.getKey());
        if (otherFreq != null) dotProduct += entry.getValue() * otherFreq;
      }

      float similarity = (float) (dotProduct / (referenceL2Norm * l2Norm));

      if (Float.isInfinite(similarity) || Float.isNaN(similarity)) this.target.score = 0.f;
      else this.target.score = similarity;
