...
public class EnglishAnalyzer extends LanguageAnalyzer {
  ...
  protected TokenStreamComponents createComponents(String fieldName, Reader reader) {
    ...
    if (config.removeElisions) result = new EnglishPossessiveFilter(Version.LUCENE_4_10_4, result);
    if (config.toLowerCase) result = new LowerCaseFilter(result);
    if (config.filterStopWords) result = new StopFilter(result, stopwords);
