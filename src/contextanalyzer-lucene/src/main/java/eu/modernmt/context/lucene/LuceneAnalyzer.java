...
public class LuceneAnalyzer implements ContextAnalyzer, DataListenerProvider {
  ...
  public LuceneAnalyzer(File indexPath, AnalyzerConfig config) throws IOException {
    this(
        new ContextAnalyzerIndex(new File(indexPath, "index")),
        new CorporaStorage(new File(indexPath, "storage")),
        config);
  }

  protected LuceneAnalyzer(
      ContextAnalyzerIndex index, CorporaStorage storage, AnalyzerConfig config) {
    this.index = index;
  ...
  public ContextVector getContextVector(
      UUID user, LanguageDirection direction, String query, int limit)
      throws ContextAnalyzerException {
    return getContextVector(
        user, direction, new StringCorpus(null, direction.source, query), limit);
  }

  @Override
  public ContextVector getContextVector(
      UUID user, LanguageDirection direction, File source, int limit)
      throws ContextAnalyzerException {
  ...
  @Override
  public ContextVector getContextVector(
      UUID user, LanguageDirection direction, Corpus query, int limit)
      throws ContextAnalyzerException {
    try {
      return this.index.getContextVector(user, direction, query, limit);
    } catch (IOException e) {
      throw new ContextAnalyzerException(
          "Failed to calculate context-vector due an internal error", e);
    }
  }
    ...
    if (size == 0) {
      ...
      Document document =
          DocumentBuilder.newInstance(
              bucket.getOwner(), bucket.getId(), bucket.getLanguage(), reader);
  ...
  public void close() throws IOException {
    try {
      ...
      try {
        ...
        if (this.analysis != null) this.analysis.shutdown();
      }
    }
  }

  protected void runAnalysis(ExecutorService executor, long maxToleratedMisalignment, int batchSize)
      throws IOException {
    ...
    for (Bucket bucket : buckets) tasks.add(new AnalysisTask(bucket));

    if (executor == null) {
      for (AnalysisTask task : tasks) task.run();
    } else {
      List<Future<Void>> results = new ArrayList<>(tasks.size());
      for (AnalysisTask task : tasks) results.add(executor.submit(task, null));
  ...
  private class AnalysisThread extends Thread {
    ...
    public AnalysisThread(AnalyzerConfig config) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug(
            "AnalysisThread started with: timeout="
                + timeout
                + ", batchSize="
                + batchSize
                + ", maxToleratedMisalignment="
                + maxToleratedMisalignment
                + ", threads="
                + threads);
      }
