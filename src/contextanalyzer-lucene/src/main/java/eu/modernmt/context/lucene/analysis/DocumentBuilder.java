...
public class DocumentBuilder {
  ...
  public static Document newInstance(LanguageDirection direction, Corpus corpus)
      throws IOException {
    return newInstance(null, 0L, direction, corpus);
  }

  public static Document newInstance(
      UUID owner, long memory, LanguageDirection direction, Corpus corpus) throws IOException {
    return newInstance(owner, memory, direction, corpus.getRawContentReader());
  }

  public static Document newInstance(
      UUID owner, long memory, LanguageDirection direction, Reader contentReader) {
  ...
  public static long getMemory(String docId) {
    ...
    if (parts.length != 3) throw new IllegalArgumentException("Invalid Document ID: " + docId);

    return Long.parseLong(parts[0]);
  }

  public static String getLanguageForContentField(String field) {
    if (!field.startsWith(CONTENT_PREFIX_FIELD)) return null;
