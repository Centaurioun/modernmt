...
public class TestLuceneAnalyzer_getContextVector {
  ...
  public void setup() throws Throwable {
    ...
    DummyBilingualCorpus enItHelloWorld =
        TestData.corpus("none", EN__IT, enHelloWorld, itHelloWorld);
    DummyBilingualCorpus enFrHelloWorld =
        TestData.corpus("none", EN__FR, enHelloWorld, frHelloWorld);
  ...
  private static boolean contains(ContextVector result, long memory) {
    for (ContextVector.Entry entry : result) {
      if (entry.memory.getId() == memory) return true;
    }

    return false;
  }

  private void test(LanguageDirection lang, String query, int... memories)
    ...
    if (memories != null) {
      for (int memory : memories) assertTrue(contains(result, memory));
    }
  }
