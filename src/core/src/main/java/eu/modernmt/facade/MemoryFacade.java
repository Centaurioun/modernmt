...
public class MemoryFacade {
  ...
  private final ChainedMultilingualCorpusFilter contributionFilter =
      CorporaCleaning.makeMultilingualFilter(
  ...
  public ImportJob add(long memoryId, TranslationUnit tu)
      throws BinaryLogException, PersistenceException {
    if (tu.timestamp == null) tu.timestamp = new Date();
  ...
  public ImportJob replace(
      long memoryId, TranslationUnit tu, String previousSentence, String previousTranslation)
      throws BinaryLogException, PersistenceException {
    TranslationUnit previous =
        new TranslationUnit(tu.tuid, tu.language, previousSentence, previousTranslation);
  ...
  private ImportJob replace(long memoryId, TranslationUnit current, TranslationUnit previous)
      throws BinaryLogException, PersistenceException {
    // Normalizing
    if (previous != null) contributionFilter.normalize(previous);
      ...
      if (previous == null)
        ...
        job =
            binlog.replace(
                ...
                previous.source,
                previous.target,
                BinaryLog.CONTRIBUTIONS_CHANNEL_ID);
  ...
  public ImportJob add(long memoryId, MultilingualCorpus corpus)
      throws PersistenceException, BinaryLogException {
    Connection connection = null;
    Database db = ModernMT.getNode().getDatabase();
    ...
    if (job == null) throw new EmptyCorpusException();
  ...
  public ImportJob getImportJob(UUID id) throws PersistenceException {
    ...
    List<NodeInfo> nodes =
        ModernMT.getNode().getClusterNodes().stream()
    ...
    for (NodeInfo node : nodes) {
      ...
      if (nodeOffset == null) nodeOffset = 0L;

      if (nodeOffset >= end) completed++;
      else minOffset = Math.min(minOffset, nodeOffset);
    }

    int quota = nodes.size() < 3 ? 1 : Math.round((2.f * nodes.size()) / 3.f);

    if (completed >= quota) job.setProgress(1.f);
    else if (begin == end) job.setProgress(0.f);
    else job.setProgress(Math.max(0.f, minOffset - begin) / (float) (end - begin));
