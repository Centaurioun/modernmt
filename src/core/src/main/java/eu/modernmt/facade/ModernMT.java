...
public class ModernMT {
  ...
  public static ClusterNode getNode() {
    if (node == null)
      throw new IllegalStateException(
          "ModernMT node not available. You must must call runForever() first.");
  ...
  public static void start(NodeConfig config, ClusterNode.StatusListener listener)
    ...
    if (listener != null) node.addStatusListener(listener);
  ...
  public static ServerInfo info(boolean localhostOnly) {
    ...
    Collection<NodeInfo> nodes =
        localhostOnly ? Collections.singleton(node.getLocalNode()) : node.getClusterNodes();
    ...
    return new ServerInfo(
        new ServerInfo.ClusterInfo(nodes),
        new ServerInfo.BuildInfo(BUILD_VERSION, BUILD_NUMBER),
        memorySize);
  }
