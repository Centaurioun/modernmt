...
public class KafkaPacket {
  ...
  public static KafkaPacket createOverwrite(
      UUID owner,
      long memory,
      TranslationUnit tu,
  ...
  public TranslationUnitMessage asTranslationUnit(
      LanguageDirection language, Sentence sSentence, Sentence sTranslation, Alignment alignment) {
    if (channel < 0 || position < 0)
      throw new IllegalStateException("Call setChannelInfo() before parsing methods.");
    ...
    TranslationUnit tu =
        new TranslationUnit(
            this.tuid, this.direction, this.sentence, this.translation, this.timestamp);
  ...
  public static KafkaPacket fromBytes(byte[] data) {
    ...
    if (type == TYPE_DELETION) return createDeletion(owner, memory);

    final LanguageDirection language =
        new LanguageDirection(buffer.readLanguage(), buffer.readLanguage());
  ...
  public byte[] toBytes() {
    ...
    buffer.write(type).write(memory).write(owner);

    if (type == TYPE_DELETION) return buffer.toArray();
    ...
    if (type == TYPE_OVERWRITE_BY_VALUE) {
      buffer.write(previousSentence).write(previousTranslation);
    }
