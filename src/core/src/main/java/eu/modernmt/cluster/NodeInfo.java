...
public class NodeInfo {
  ...
  private static final String TRANSLATION_DIRECTIONS_ATTRIBUTE =
      "NodeInfo.TRANSLATION_DIRECTIONS_ATTRIBUTE";
  ...
  static NodeInfo fromMember(Member member) {
    ...
    Map<Short, Long> positions =
        deserializeChannels(member.getStringAttribute(DATA_CHANNELS_ATTRIBUTE));
  ...
  private NodeInfo(
      String uuid, ClusterNode.Status status, Map<Short, Long> channels, String address) {
    this.uuid = uuid;
    this.status = status;
  ...
  static boolean statusIs(Member member, ClusterNode.Status... statuses) {
    ...
    for (ClusterNode.Status status : statuses) {
      if (status.toString().equalsIgnoreCase(memberStatus)) return true;
    }
  ...
  static boolean hasTranslationDirection(Member member, LanguageDirection direction) {
    ...
    String search =
        '[' + direction.source.toLanguageTag() + ':' + direction.target.toLanguageTag() + ']';
  ...
  private static ClusterNode.Status deserializeStatus(String encoded) {
    if (encoded == null) return ClusterNode.Status.UNKNOWN;
  ...
  private static HashMap<Short, Long> deserializeChannels(String encoded) {
    if (encoded == null || encoded.isEmpty()) return new HashMap<>();
