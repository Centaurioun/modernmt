...
/**
 * Created by davide on 06/09/16. Updated by andrearossi on 29/03/17
 *
 * <p>This class manages the Apache Kafka environment: it handles the Kafka channels, topics and
 * partitions, it creates the kafka producer and consumer for sending and reading messages, and
 * starts separate threads to connect to the Apache Kafka server to perform polling in order to find
 * the proper positions on the topics
 */
public class KafkaBinaryLog implements BinaryLog {
  ...
  public KafkaBinaryLog(
      LanguageIndex languages,
      Preprocessor preprocessor,
      Aligner aligner,
      String uuid,
      BinaryLogConfig config) {
    this.uuid = uuid;
    ...
    this.channels[0] =
        new KafkaChannel(
            BinaryLog.MEMORY_UPLOAD_CHANNEL_ID, topicNames[BinaryLog.MEMORY_UPLOAD_CHANNEL_ID]);
    this.channels[1] =
        new KafkaChannel(
            BinaryLog.CONTRIBUTIONS_CHANNEL_ID, topicNames[BinaryLog.CONTRIBUTIONS_CHANNEL_ID]);
  ...
  /**
   * This method calculates default acceptable names for all kafka topics and puts them in an
   * ordered String array. The array order is: 0: memories topic name 1: contributions topic name
    ...
    if (prefix != null) {
      ...
      int length =
          Math.max(
                  ...
                  normalizedPrefix.length() + contributionsTopicName.length())
              + 1;
      ...
      if (length > maxLength)
        normalizedPrefix =
            normalizedPrefix
                .substring(0, length - maxLength - 1)
  ...
  public static Properties loadProperties(String filename, String[] hosts, int port) {
    ...
    try {
      ...
      for (int i = 0; i < servers.length; i++) servers[i] = hosts[i] + ':' + port;
  ...
  /**
   * This method makes this KafkaBinaryLog connect to the Kafka server specified in its initial
   * BinaryLogConfig. To connect, a default timeout of 60 seconds is employed. If the connection
   * succeeded, this method returns a map that associates to the (short) ID of each kafka topic the
   * corresponding (long) last written position.
   *
   * @return a map containing, for each Kafka topic, its latest written positions if a consumer was
   *     to be started; null if only the producer was to be started.
  ...
  /**
   * This method makes this KafkaBinaryLog connect to the Kafka server specified in its initial
   * BinaryLogConfig. It launches both a kafka consumer and a kafka producer to interact with the
   * Kafka server.
   *
   * <p>The connection process will use the passed timeout and timeunit values.
   *
   * <p>If the connection succeeds, this method returns a map that associates to the (short) ID of
   * each kafka topic the corresponding (long) last written position.
   *
   ...
   * BinaryLogConfig.
   *
   * <p>A kafka consumer and/or a kafka producer will be launched depending on the passed "enable"
   * params. If both the "enable" params are false, this method will do nothing.
   *
   * <p>The connection process will use the passed timeout and timeunit values.
   *
   * <p>If the connection succeeds, this method returns a map that associates to the (short) ID of
   ...
   * @param timeout timeout to use while connecting to the Kafka server
   * @param unit time unit for the timeout
   * @param enableConsumer boolean value that specifies whether the kafka consumer should be started
   *     or not
   * @param enableProducer boolean value that specifies whether the kafka producer should be started
   *     or not
   * @return a map containing, for each Kafka topic, its latest written positions if a consumer was
   *     to be started; null if only the producer was to be started.
   * @throws HostUnreachableException if could not connect to the Kafka server
   */
  @Override
  public Map<Short, Long> connect(
      long timeout, TimeUnit unit, boolean enableConsumer, boolean enableProducer)
    ...
    // Create Kafka consumer and connect to the Kafka remote server to get the latest positions for
    // each channel
    if (enableConsumer) {
      // load consumer properties and build kafka consumer for reading messages from the server from
      ...
      if (connectThread
          .isAlive()) // if the thread is still alive could not connect to the Kafka server
  ...
  @Override
  public ImportJob upload(Memory memory, MultilingualCorpus corpus, short channel)
      throws BinaryLogException {
    return upload(memory, corpus, getLogChannel(channel));
  }

  @Override
  public ImportJob upload(Memory memory, MultilingualCorpus corpus, LogChannel channel)
      throws BinaryLogException {
    if (this.producer == null) throw new IllegalStateException("connect() not called");

    if (logger.isDebugEnabled()) logger.debug("Uploading memory " + memory);
    ...
    try {
      ...
      importEnd =
          importBegin =
              sendElement(
                  KafkaPacket.createAddition(memory.getOwner(), memory.getId(), tu), true, channel);
      size++;
      ...
      while (tu != null) {
        ...
        if (tu == null)
          importEnd =
              sendElement(
                  KafkaPacket.createAddition(memory.getOwner(), memory.getId(), current),
                  true,
                  channel);
        else
          sendElement(
  ...
  @Override
  public ImportJob upload(Memory memory, TranslationUnit tu, short channel)
      throws BinaryLogException {
    return upload(memory, tu, getLogChannel(channel));
  }

  @Override
  public ImportJob upload(Memory memory, TranslationUnit tu, LogChannel channel)
      throws BinaryLogException {
    if (this.producer == null) throw new IllegalStateException("connect() not called");
    long offset =
        sendElement(
  ...
  @Override
  public ImportJob replace(Memory memory, TranslationUnit tu, short channel)
      throws BinaryLogException {
    return replace(memory, tu, getLogChannel(channel));
  }

  @Override
  public ImportJob replace(Memory memory, TranslationUnit tu, LogChannel channel)
      throws BinaryLogException {
    if (this.producer == null) throw new IllegalStateException("connect() not called");

    long offset =
        sendElement(
            KafkaPacket.createOverwrite(memory.getOwner(), memory.getId(), tu), true, channel);
    return ImportJob.createEphemeralJob(memory.getId(), offset, channel.getId());
  }

  @Override
  public ImportJob replace(
      Memory memory,
      TranslationUnit tu,
  ...
  public ImportJob replace(
    ...
    long offset =
        sendElement(
            KafkaPacket.createOverwrite(
                memory.getOwner(), memory.getId(), tu, previousSentence, previousTranslation),
            true,
            channel);
    return ImportJob.createEphemeralJob(memory.getId(), offset, channel.getId());
  }
  ...
  private long sendElement(KafkaPacket packet, boolean sync, LogChannel channel)
      throws BinaryLogException {
    pollingThread.ensureRunning();

    Future<RecordMetadata> future =
        producer.send(new ProducerRecord<>(channel.getName(), 0, packet));
    ...
    if (sync) {
      try {
        ...
        else throw new BinaryLogException("Unexpected exception while uploading", cause);
      }
    }
  ...
  public void close() throws IOException {
    ...
    try {
      if (!pollingThread.awaitTermination(TimeUnit.SECONDS, 2)) pollingThread.shutdownNow();
    } catch (InterruptedException e) {
