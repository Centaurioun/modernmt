...
class KafkaDataBatch implements DataBatch {
  ...
  public KafkaDataBatch(
      LanguageIndex languageIndex,
      Preprocessor preprocessor,
      Aligner aligner,
      KafkaBinaryLog manager) {
  ...
  private DataPartition getDataPartition(LanguageDirection direction, int expectedSize) {
    if (cachedPartitions.isEmpty()) return new DataPartition().reset(direction, expectedSize);
    else return cachedPartitions.pop().reset(direction, expectedSize);
  }
  ...
  public void load(ConsumerRecords<Integer, KafkaPacket> records, boolean process, boolean align)
      throws ProcessingException, AlignerException, InterruptedException {
    // Load records
    ...
    for (ConsumerRecord<Integer, KafkaPacket> record : records) {
      ...
      if (packetType == KafkaPacket.TYPE_DELETION) {
        ...
        if (direction == null)
          direction =
              LanguageCache.defaultMapping(
                  packet.getDirection()); // No specific rule in languageIndex

        DataPartition partition =
            cachedDataSet.computeIfAbsent(direction, key -> getDataPartition(key, size));
  ...
  private class DataPartition {
    ...
    public void process(boolean process, boolean align, Collection<TranslationUnitMessage> output)
        throws ProcessingException, AlignerException, InterruptedException {
      ...
      if (process || align) {
        ...
        if (align) alignments = aligner.getAlignments(direction, sourceSentences, targetSentences);
