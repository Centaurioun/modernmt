...
public class ClusterNode {
  ...
  public Engine getEngine() {
    if (engine == null)
      throw new IllegalStateException(
          "ClusterNode not ready. Call bootstrap() to initialize the member.");
    return engine;
  }

  public BinaryLog getBinaryLog() {
    if (binaryLog == null) throw new UnsupportedOperationException("BinaryLog unavailable");
    return binaryLog;
  }

  public Database getDatabase() {
    if (database == null) throw new IllegalStateException("Database unavailable.");
    return database;
  }
        ...
        if (expected == null || contains(expected, this.status)) {
          ...
          for (StatusListener listener : statusListeners) {
            try {
              ...
              logger.error(
                  "Unexpected exception while updating Node status. Resuming normal operations.",
                  e);
            }
          }
  ...
  private static void addToBinaryLog(LogDataListener listener, BinaryLog binlog) {
    if (listener != null) binlog.addLogDataListener(listener);
  }
  ...
  private Config getHazelcastConfig(NodeConfig nodeConfig, long interval, TimeUnit unit) {
    ...
    hazelcastConfig.setGroupConfig(new GroupConfig().setName(this.clusterName));
    ...
    if (host != null) hazelcastConfig.getNetworkConfig().setPublicAddress(host);
  ...
  public void start(NodeConfig nodeConfig, long joinTimeoutInterval, TimeUnit joinTimeoutUnit)
      throws FailedToJoinClusterException, BootstrapException {
    logger.info(
        "Starting node in cluster \""
            + this.clusterName
            + "\" with config:\n"
            + nodeConfig.toString());
    ...
    logger.info(
        "Node joined cluster of "
            + countClusterMembers(false)
            + " members in "
            + (timer.time() / 1000.)
            + "s");
      ...
      this.engine
          ...
          .setListener(
              new DecoderListener() {
                ...
                @Override
                public void onDecoderAvailabilityChanged(
                    int currentAvailability, int maxAvailability) {
    ...
    if (binaryLogConfig.isEnabled()) {
      ...
      try {
        ...
        if (countClusterMembers(true) > 0) {
          ...
          logger.info(
              "Binary log sync running in background, force single running node cluster start");
        }
    ...
    if (databaseConfig.isEnabled()) {
      ...
      switch (databaseConfig.getType()) {
        ...
        default:
          throw new Error(
              "Invalid value for enum DatabaseConfig.Type: " + databaseConfig.getType());
      }
      ...
      try {
        ...
        throw new BootstrapException(
            "Failed to create database: " + this.database.getClass().getSimpleName(), e);
      }
    }
    ...
    translationService =
        hazelcast.getDistributedObject(TranslationService.SERVICE_NAME, "TranslationService");
  ...
  private int countClusterMembers(boolean onlyRunning) {
    ...
    for (Member member : hazelcast.getCluster().getMembers()) {
      if (!onlyRunning || NodeInfo.statusIs(member, Status.RUNNING)) size++;
    }
  ...
  private void updateDecoderAvailability(int currentAvailability, int maxAvailability) {
    if (currentAvailability == 0) setStatus(Status.UNAVAILABLE, Status.RUNNING, Status.DEGRADED);
    else if (currentAvailability < maxAvailability)
      setStatus(Status.DEGRADED, Status.RUNNING, Status.UNAVAILABLE);
    else setStatus(Status.RUNNING, Status.UNAVAILABLE, Status.DEGRADED);
  }
  ...
  private Member getRandomMember(LanguageDirection language, LanguageBridge bridge)
    ...
    for (Member member : members) {
      if (!NodeInfo.statusIs(member, Status.RUNNING, Status.DEGRADED)) continue;

      activeNodes++;

      if (hasTranslationDirection(member, language, bridge)) candidates.add(member);
    }

    if (candidates.isEmpty()) {
      if (activeNodes > 0) throw new UnsupportedLanguageException(language);
      else throw new DecoderUnavailableException("Could not find active node in the cluster");
    }
  ...
  private static boolean hasTranslationDirection(
      Member member, LanguageDirection language, LanguageBridge bridge) {
    if (bridge == null) return NodeInfo.hasTranslationDirection(member, language);
    else
      return NodeInfo.hasTranslationDirection(member, bridge.source)
          && NodeInfo.hasTranslationDirection(member, bridge.target);
  }
  ...
  public boolean awaitTermination(long timeout, TimeUnit unit) throws InterruptedException {
    if (getStatus() == Status.SHUTDOWN) unit.timedJoin(shutdownThread, timeout);
