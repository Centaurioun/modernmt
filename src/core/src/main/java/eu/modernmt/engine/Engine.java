...
public class Engine implements Closeable, DataListenerProvider {
  ...
  public static Engine load(EngineConfig config) throws BootstrapException {
    ...
    return new Engine(
        name, languageIndex, aligner, preprocessor, postprocessor, contextAnalyzer, decoder);
  }
  ...
  public Decoder getDecoder() {
    if (decoder == null) throw new UnsupportedOperationException("Decoder unavailable");

    return decoder;
  }

  public Aligner getAligner() {
    if (aligner == null) throw new UnsupportedOperationException("Aligner unavailable");
  ...
  private static void addDataListener(Object object, ArrayList<LogDataListener> listeners) {
    ...
    if (object instanceof LogDataListener) listeners.add((LogDataListener) object);
  ...
  public void close() throws IOException {
    ...
    try {
      if (contextAnalyzer != null) contextAnalyzer.close();
    } catch (IOException e) {
