...
public class PreprocessingPipeline {
  ...
  public PreprocessingPipeline(
      LanguageDirection language, CorporaPartition mainPartition, int threads) {
  ...
  public void process(Collection<MultilingualCorpus> corpora)
    ...
    try {
      Map<LanguageDirection, Long> bilingualCorporaLinesMap =
          Corpora.countLines(maskedMultilingualCorpora, threads);
      ...
      for (MultilingualCorpus corpus : maskedMultilingualCorpora) {
        for (LanguageDirection language : corpus.getLanguages()) {
          ...
          double weight =
              PartitioningUtils.getAdjustedWeight(
                  language, corpus, extraPartitionsLines, bilingualCorporaLines);
          int lineCount = corpus.getLineCount(language);

          PreprocessingTask sourceTask =
              new PreprocessingTask(
                  preprocessor,
                  language,
                  corpus.getCorpus(language, true),
                  lineCount,
                  mainPartition);
          PreprocessingTask targetTask =
              new PreprocessingTask(
                  preprocessor,
                  language.reversed(),
                  corpus.getCorpus(language, false),
                  lineCount,
                  mainPartition);
