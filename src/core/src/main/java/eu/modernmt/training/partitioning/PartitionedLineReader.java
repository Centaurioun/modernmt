...
public class PartitionedLineReader implements LineReader {
  ...
  public PartitionedLineReader(Corpus corpus, int lines, List<PartitionWriter> partitions)
      throws IOException {
    this.reader = corpus.getContentReader();
    this.partitions = partitions;

    int extraLines = 0;
    for (PartitionWriter partition : partitions) extraLines += partition.size();
