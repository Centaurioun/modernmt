...
public class Parameters {
  ...
  public Boolean getBoolean(String name) throws ParameterParsingException {
    ...
    if (result == null) throw new ParameterParsingException(name);
  ...
  public Boolean getBoolean(String name, Boolean def) throws ParameterParsingException {
    ...
    else if (value.equals("false") || value.equals("0")) return false;
    else throw new ParameterParsingException(name, value);
  }
  ...
  public String getString(String name, boolean canBeEmpty, String def)
      throws ParameterParsingException {
    String value = req.getParameter(name);
    if (value == null) return def;

    if (!canBeEmpty && value.trim().isEmpty()) throw new ParameterParsingException(name, value);
  ...
  public Language[] getLanguageArray(String name) throws ParameterParsingException {
    ...
    for (int i = 0; i < rawArray.length; i++) array[i] = Language.fromString(rawArray[i]);
  ...
  /**
   * Get the language pair from the request parameter using the passed source and target language
   * parameter names.
   *
   * <p>If neither the source language parameter nor the target language parameter are passed,
   * return a default LanguageDirection.
   *
   ...
   * @param def the default LanguageDirection to return if neither sourceName nor targetName can be
   *     parsed from the request
   * @return language pair build from the values of parameters sourceName and targetName, if they
   *     are passed, or the default LanguageDirection otherwise.
   * @throws ParameterParsingException
   */
  public LanguageDirection getLanguagePair(
      String sourceName, String targetName, LanguageDirection def)
      throws ParameterParsingException {
   ...
   * @throws ParameterParsingException if the parameter with name sourceName and/or the parameter
   *     with name targetName can not be found in the request
   */
  public LanguageDirection getLanguagePair(String sourceName, String targetName)
      throws ParameterParsingException {
