...
public class ApiServer {
  ...
  static {
    JSONSerializer.registerCustomSerializer(
        TranslationResponse.class, new TranslationResponseSerializer());
    JSONSerializer.registerCustomSerializer(Alignment.class, new AlignmentSerializer());
    JSONSerializer.registerCustomSerializer(
        ContextVectorResult.class, new ContextVectorResultSerializer());
  ...
  public ApiServer(ServerOptions options) {
    ...
    MultipartConfigInjectionHandler multipartWrapper =
        new MultipartConfigInjectionHandler(
            options.temporaryDirectory,
            options.maxFileSize,
            options.maxRequestSize,
            options.fileSizeThreshold);
  ...
  private String normalizeContextPath(String contextPath) {
    if (contextPath == null || contextPath.trim().isEmpty()) return null;
  ...
  public void stop() throws Exception {
    while (requestPool.getQueueSize() > 0) Thread.sleep(1000);
