...
public class RESTRequest {
  ...
  public Class<? extends Action> getActionClass() {
    if (template != null) return template.getActionClass();
    else return null;
  }
  ...
  private boolean isContentType(String contentType) {
    ...
    for (String token : found.split(";")) if (contentType.equalsIgnoreCase(token)) return true;
    ...
    if (queryString == null) {
      ...
      if (qs == null || qs.isEmpty()) {
        ...
        if (isContentType("application/x-www-form-urlencoded"))
          try {
            qs = IOUtils.toString(request.getInputStream(), encoding);
          } catch (IOException e) {
            // Ignore it
          }
      }
  ...
  private String getJSONContent() {
    if (json == null) {
      ...
      if (isContentType("application/json")) {
        try {
          this.json = IOUtils.toString(request.getInputStream(), encoding);
        } catch (IOException e) {
        }
      }
    }
  ...
  private void ensureParameters() throws Parameters.ParameterParsingException {
    ...
    if (!queryString.isEmpty()) {
      ...
      for (String param : params) {
        ...
        if (tokens.length == 2) {
          try {
            this.parameters.put(tokens[0], URLDecoder.decode(tokens[1], encoding).trim());
      ...
      try {
        ...
        throw new Parameters.ParameterParsingException(
            "Failed to retrieve multipart/form-data request parts", e);
  ...
  public String getPathParameter(String varname) throws TemplateException {
    ...
    if (i < 0) throw new TemplateException(varName);

    String value = pathTokens.get(i).trim();
    if (value.isEmpty()) throw new TemplateException(varName);
  ...
  public JsonObject getJSONObject() {
    if (jsonObject == null) {
      ...
      if (parsed == null) this.jsonObject = NULL_OBJECT;
      else this.jsonObject = parsed;
    }

    if (jsonObject == NULL_OBJECT) return null;
  ...
  public JsonArray getJSONArray() {
    if (jsonArray == null) {
      ...
      if (parsed == null) this.jsonArray = NULL_ARRAY;
      else this.jsonArray = parsed;
    }

    if (jsonArray == NULL_ARRAY) return null;
