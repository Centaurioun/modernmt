...
public class TranslationResponseSerializer implements JsonSerializer<TranslationResponse> {
  ...
  public JsonElement serialize(
      TranslationResponse src, Type typeOfSrc, JsonSerializationContext context) {
    ...
    if (src.verbose) {
      ...
      json.add(
          "alignment", context.serialize(src.translation.getSentenceAlignment(), Alignment.class));
    }
  ...
  private static JsonElement serializeHypothesis(
      JsonSerializationContext context, Translation translation, boolean verbose) {
    JsonObject json = new JsonObject();
  ...
  private static JsonArray serializeTokens(Sentence sentence) {
    JsonArray array = new JsonArray();
    for (Token token : sentence) array.add(token.toString());
    return array;
  }
}
