...
@Route(
    aliases = {"memories/:id/corpus", "domains/:id/corpus"},
    method = HttpMethod.POST)
public class AddToMemoryCorpus extends ObjectAction<ImportJob> {

  @Override
  protected ImportJob execute(RESTRequest req, Parameters _params)
      throws BinaryLogException, PersistenceException {
    Params params = (Params) _params;

    if (params.tu != null) return ModernMT.memory.add(params.memory, params.tu);
    else return ModernMT.memory.add(params.memory, params.corpus);
  }
  ...
  public static class Params extends Parameters {
      ...
      if (source == null && target == null) {
        ...
        switch (fileType) {
          ...
          case PARALLEL:
            ...
            corpus =
                new ParallelFileCorpus(
                    language, getFileProxy("source", gzipped), getFileProxy("target", gzipped));
            break;
          default:
        ...
        if (source == null) throw new ParameterParsingException("sentence");
        if (target == null) throw new ParameterParsingException("translation");
    ...
    private FileProxy getFileProxy(String prefix, boolean gzipped)
