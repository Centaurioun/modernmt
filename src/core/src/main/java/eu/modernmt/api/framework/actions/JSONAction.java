...
public abstract class JSONAction implements Action {
  ...
  public final void execute(RESTRequest req, RESTResponse resp) {
    try {
      ...
      if (logger.isDebugEnabled())
        logger.debug(
            "Language direction '" + e.getLanguageDirection() + "' is not supported " + this, e);
      resp.badRequest(e);
      ...
      if (logger.isDebugEnabled()) logger.debug("Failed to import empty or poor quality corpus", e);
      resp.badRequest(e);
    } catch (Throwable e) {
  ...
  protected abstract JSONActionResult getResult(RESTRequest req, Parameters params)
      throws Throwable;
